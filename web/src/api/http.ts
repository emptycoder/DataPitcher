...
export type JsonRequest = Readonly<Omit<RequestInit, 'body' | 'headers'>> & Readonly<{
  body?: unknown;
  headers?: HeadersInit;
}>;
...
export async function requestJson<T>(url: string, authentication: AuthenticationAdapter, options: JsonRequest = {}): Promise<T> {
  ...
  const response = await fetch(url, { ...init, headers, body: body === undefined ? undefined : JSON.stringify(body) });
