...
export function PlanBuilderScreen({ planId }: Readonly<{ planId: string | null }>) {
    ...
    const warnings = useMemo(() => {
        ...
        if (sourceId && targetId && sourceId === targetId)
            list.push(
                'Source and target are the same connection. The transfer would write back into the database it reads from.',
            );
        if (selectionEntry?.connectionId && sourceId && selectionEntry.connectionId !== sourceId)
            list.push(
                'The selection was authored against a different connection than the chosen source. Sealing requires them to match.',
            );
        if (source && target && source.providerId !== target.providerId)
            list.push(
        ...
        <>
            ...
            <form className="grid gap-5 lg:grid-cols-[1fr_360px]" onSubmit={submit}>
                <div className="grid content-start gap-5">
                        ...
                        <div className="grid gap-4">
                            <Field label="Plan name" required>
                                <TextInput
                                    ...
                                    placeholder="e.g. Customer 42 to staging"
                                    value={name}
                                />
                            </Field>
                            <Field hint="Shown to reviewers alongside the plan." label="Operator note">
                                <TextArea
                                    ...
                                    placeholder="Why this transfer is happening, ticket references, caveats…"
                                    rows={3}
                                    value={note}
                                />
                            </Field>
                    ...
                    <Card>
                        <CardHeader
                            description="Which root rows to transfer."
                            ...
                            <Select onChange={(event) => setSelectionId(event.target.value)} value={selectionId}>
                                ...
                                {(selections.data ?? []).map((selection) => (
                                    <option key={selection.selectionId} value={selection.selectionId}>
                                        {selectionNames[selection.selectionId]?.name ||
                                            selection.displayName ||
                                            'Untitled selection'}{' '}
                                        · {selectionNames[selection.selectionId]?.rootTable ?? selection.mode} ·{' '}
                    ...
                    <Card>
                        <CardHeader
                            description="Rows are read from the source and written into the target."
                    ...
                    <Card>
                        ...
                        <div className="grid gap-3">
                            <SummaryRow
                                label="Selection"
                                value={
                                    selectionEntry?.name || (selectionId ? `Selection ${shortId(selectionId)}` : null)
                                }
                            />
                        ...
                        <p className="mt-3 text-xs text-fg-faint">
                            Creating a plan does not touch either database. Sealing reads both; only a started transfer
                            writes.
                        </p>
                    </Card>
                </div>
...
function ConnectionSelect({
    ...
    onChange,
    emphasis,
}: Readonly<{ label: string; value: string; onChange: (id: string) => void; emphasis?: boolean }>) {
    const connections = useConnections();
    return (
...
function SummaryConnection({
    label,
    connection,
    tone,
}: Readonly<{
    label: string;
    connection: Readonly<{ displayName: string; providerId: string; health: string }> | null;
    tone: 'accent' | 'warning';
}>) {
    return (
        <div
            className={cx(
                ...
                tone === 'warning' ? 'border-warning/50 bg-warning-soft/40' : 'border-border',
            )}
        >
            {connection ? (
                <ProviderMark providerId={connection.providerId} size="sm" />
            ) : (
                <span className="size-7 rounded-lg bg-surface-3" />
            )}
            <div className="min-w-0 flex-1">
                <div className="text-[11px] font-semibold tracking-wide text-fg-muted uppercase">{label}</div>
                <div className={cx('truncate text-sm', connection ? 'font-medium text-fg' : 'text-fg-faint')}>
                    {connection?.displayName ?? 'Not chosen'}
                </div>
            </div>
            {connection ? (
                <Badge dot tone={connection.health === 'Healthy' ? 'success' : 'warning'}>
                    {connection.health}
                </Badge>
            ) : null}
        </div>
