...
public sealed class PostgreSqlTransferModelsTests : IClassFixture<PostgreSqlClosureFixture>
{
    ...
    {
        var columns = new[] { new PostgreSqlWriteColumn("code", "text", NpgsqlTypes.NpgsqlDbType.Text, false, false, false, false, "C") };
        Assert.Throws<ArgumentException>(() => new PostgreSqlWriteTable(new TableAddress("dp", "no_key_rows"), columns));
    }
    ...
    {
        var table = new PostgreSqlWriteTable(new TableAddress("dp", "composite_stable_rows"), [
    ...
    {
        var table = new PostgreSqlWriteTable(new TableAddress("dp", "uuid_key_rows"), [new("id", "uuid", NpgsqlTypes.NpgsqlDbType.Uuid, true, false, false, false, null)]);
    ...
    public async Task ReadAsync_WhenAColumnTypeIsNotSupported_ThrowsNotSupportedException()
    {
        ...
        var error = await Assert.ThrowsAsync<NotSupportedException>(() => new PostgreSqlTransferSchemaReader(scope.Target).ReadAsync(scope.Schema, "transfer_rows", ["id"], CancellationToken.None));
