...
public sealed class PostgreSqlCommandRecorder : ILoggerFactory, ILogger
{
    ...
    public int Count(string tag, TableDefinition? table = null) =>
        _messages.Count(message =>
            message.StartsWith("Executing command", StringComparison.Ordinal)
            && message.Contains(tag, StringComparison.Ordinal)
            && (
                table is null
                || message.Contains(PostgreSqlIdentifier.Qualified(table.Schema, table.Name), StringComparison.Ordinal)
            )
        );
    ...
    private static bool ContainsLargeInList(string message, int threshold)
    {
        ...
        while (true)
        {
            ...
            if (close < 0)
                return false;
            if (message[(index + 5)..close].Split(',').Length > threshold)
                return true;
            searchStart = close + 1;
        }
    }
