...
public sealed class PostgreSqlClosureFixture : IAsyncLifetime
{
    ...
    public async Task<PostgreSqlClosureScope> CreateScopeAsync(
        PostgreSqlCommandRecorder? targetRecorder = null,
        PostgreSqlCommandRecorder? sourceRecorder = null
    )
    {
    ...
    {
        var connectionString = new NpgsqlConnectionStringBuilder(_target.GetConnectionString())
        {
            SearchPath = schema,
        }.ConnectionString;
        if (recorder is null)
    ...
    {
        var connectionString = new NpgsqlConnectionStringBuilder(_source.GetConnectionString())
        {
            SearchPath = schema,
        }.ConnectionString;
        if (recorder is null)
...
public sealed class PostgreSqlClosureScope(string schema, NpgsqlDataSource source, NpgsqlDataSource target)
    : IAsyncDisposable
{
    ...
    public static async Task CreateAsync(NpgsqlDataSource dataSource, string schema, bool target)
    {
        ...
        foreach (var sql in SchemaSql(target))
            await ExecuteOnAsync(dataSource, sql);
    }
