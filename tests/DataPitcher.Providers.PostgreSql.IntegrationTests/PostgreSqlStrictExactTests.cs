...
public sealed class PostgreSqlStrictExactTests : IClassFixture<PostgreSqlClosureFixture>
{
    ...
    public async Task EnsureAvailableAsync_WhenTargetHasUserTrigger_RefusesStrictExact()
    {
        ...
        await scope.ExecuteTargetAsync(
            "CREATE TABLE transfer_rows (id integer PRIMARY KEY, code text NOT NULL); CREATE FUNCTION transfer_notice() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RETURN NEW; END; $$; CREATE TRIGGER transfer_trigger BEFORE INSERT ON transfer_rows FOR EACH ROW EXECUTE FUNCTION transfer_notice();"
        );

        await Assert.ThrowsAsync<PostgreSqlStrictExactBlockedException>(() =>
            new PostgreSqlStrictExact(scope.Target).EnsureAvailableAsync(
                TransferTable(scope.Schema),
    ...
    public async Task EnsureAvailableAsync_WhenTargetHasRewriteRule_RefusesStrictExact()
    {
        await using var scope = await _fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE transfer_rows (id integer PRIMARY KEY, code text NOT NULL); CREATE RULE transfer_rule AS ON INSERT TO transfer_rows DO INSTEAD NOTHING;"
        );
    ...
    public async Task EnsureAvailableAsync_WhenTargetIsReferencedByCascadingUpdateConstraint_RefusesStrictExact()
    {
        await using var scope = await _fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE transfer_rows (id integer PRIMARY KEY, code text NOT NULL); CREATE TABLE transfer_children (id integer PRIMARY KEY, parent_id integer NOT NULL REFERENCES transfer_rows(id) ON UPDATE CASCADE);"
        );
    ...
    public async Task EnsureAvailableAsync_WhenTargetHasNoSideEffects_AllowsStrictExact()
    {
        ...
        await new PostgreSqlStrictExact(scope.Target).EnsureAvailableAsync(
            TransferTable(scope.Schema),
            CancellationToken.None
        );
    }
    ...
    public async Task VerifyAsync_AfterCommit_RequiresAffectedKeysToEqualThePlannedManifest()
    {
        ...
        await strict.RecordPlannedAsync(
            context,
            table,
    ...
    public async Task RealignAsync_DiscoversTheOwnedSequenceAndAdvancesPastExplicitKeys()
    {
        await using var scope = await _fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE SEQUENCE unexpected_generator AS bigint START WITH 1; CREATE TABLE sequence_rows (id bigint PRIMARY KEY DEFAULT nextval('unexpected_generator'), code text NOT NULL); ALTER SEQUENCE unexpected_generator OWNED BY sequence_rows.id; INSERT INTO sequence_rows (id,code) VALUES (10,'ten');"
        );

        await new PostgreSqlSequenceRealigner(scope.Target).RealignAsync(
            SequenceTable(scope.Schema),
            "id",
            CancellationToken.None
        );
        ...
        Assert.Equal(
            11L,
            await scope.ScalarTargetAsync<long>("INSERT INTO sequence_rows (code) VALUES ('next') RETURNING id")
        );
    }
    ...
    public async Task RealignAsync_WhenThePositiveSequenceIsAlreadyAhead_DoesNotRewindIt()
    {
        ...
        await using (var connection = await scope.Target.OpenConnectionAsync())
        {
            ...
            await using var command = new NpgsqlCommand(
                "SELECT setval('unexpected_generator',50,true)",
                connection,
                transaction
            );
        ...
        Assert.Equal(
            51L,
            await scope.ScalarTargetAsync<long>("INSERT INTO sequence_rows (code) VALUES ('ahead') RETURNING id")
        );
    }
    ...
    public async Task RealignAsync_WhenTheOwnedSequenceDecreases_AdvancesPastTheOccupiedMinimum()
    {
        await using var scope = await _fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE SEQUENCE backwards_generator AS bigint START WITH -1 INCREMENT BY -1; CREATE TABLE sequence_rows (id bigint PRIMARY KEY DEFAULT nextval('backwards_generator'), code text NOT NULL); ALTER SEQUENCE backwards_generator OWNED BY sequence_rows.id; INSERT INTO sequence_rows (id,code) VALUES (-10,'ten');"
        );
    ...
    public async Task RealignAsync_WhenTheOwnedSequenceIsSharedByAnotherColumn_RefusesToAdjustIt()
    {
        ...
        await Assert.ThrowsAsync<NotSupportedException>(() =>
            new PostgreSqlSequenceRealigner(scope.Target).RealignAsync(
                SequenceTable(scope.Schema),
                "id",
                CancellationToken.None
            )
    ...
    public async Task RealignAsync_WhenTheOwnedSequenceCycles_RefusesToAdjustIt()
    {
        await using var scope = await _fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE SEQUENCE cycling_generator AS bigint START WITH 1 CYCLE; CREATE TABLE sequence_rows (id bigint PRIMARY KEY DEFAULT nextval('cycling_generator'), code text NOT NULL); ALTER SEQUENCE cycling_generator OWNED BY sequence_rows.id;"
        );
    ...
    public async Task RealignAsync_WhenTheOwnedSequenceTableIsEmpty_DoesNotAdvanceTheGenerator()
    {
        await using var scope = await _fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE SEQUENCE empty_generator AS bigint START WITH 1; CREATE TABLE sequence_rows (id bigint PRIMARY KEY DEFAULT nextval('empty_generator'), code text NOT NULL); ALTER SEQUENCE empty_generator OWNED BY sequence_rows.id;"
        );
        ...
        Assert.Equal(
            1L,
            await scope.ScalarTargetAsync<long>("INSERT INTO sequence_rows (code) VALUES ('first') RETURNING id")
        );
    }
    ...
    public async Task RealignAsync_WhenTheColumnHasNoOwnedSequence_LeavesItsExistingValuesAlone()
    {
        await using var scope = await _fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE sequence_rows (id bigint PRIMARY KEY, code text NOT NULL); INSERT INTO sequence_rows VALUES (10,'ten');"
        );
    ...
    private static PostgreSqlWriteTable TransferTable(string schema) =>
        new(
    ...
    private static PostgreSqlWriteTable SequenceTable(string schema) =>
        new(
            new(schema, "sequence_rows"),
            [
                new("id", "bigint", NpgsqlDbType.Bigint, true, false, false, false, null),
                new("code", "text", NpgsqlDbType.Text, false, false, false, false, "C"),
            ]
        );
