...
public sealed class PostgreSqlSelectionSqlGeneratorCoverageTests
{
    ...
    {
        var parent = new TableDefinition("sales", "parents", [new("tenant_id", typeof(int), false), new("id", typeof(int), false)], new("pk_parents", ["tenant_id", "id"]), []);
        var child = new TableDefinition("sales", "children", [new("id", typeof(int), false), new("parent_tenant_id", typeof(int), false), new("parent_id", typeof(int), false)], new("pk_children", ["id"]), []);
        ...
        Assert.Contains("\"c\".\"parent_tenant_id\" = \"p\".\"tenant_id\" AND \"c\".\"parent_id\" = \"p\".\"id\"", sql.CommandText, StringComparison.Ordinal);
    }
    ...
    public void Compile_ReverseForeignKeyJoin_PairsEachParentAndChildColumn()
    {
        ...
        Assert.Contains("\"p\".\"tenant_id\" = \"c\".\"parent_tenant_id\" AND \"p\".\"id\" = \"c\".\"parent_id\"", sql.CommandText, StringComparison.Ordinal);
    }

    [Fact]
    public void Compile_ManualJoin_PairsEverySpecifiedColumn()
    {
        var orders = new TableDefinition("sales", "orders", [new("tenant_id", typeof(int), false), new("id", typeof(int), false), new("customer_tenant_id", typeof(int), false), new("customer_id", typeof(int), false)], new("pk_orders", ["tenant_id", "id"]), []);
        var customers = new TableDefinition("sales", "customers", [new("tenant_id", typeof(int), false), new("id", typeof(int), false)], new("pk_customers", ["tenant_id", "id"]), []);
        var query = new SelectionQuery(new([orders, customers], []), new(orders, "o"), new(orders.PrimaryKey), [new ManualJoin("o", "c", customers, [new("customer_tenant_id", "tenant_id"), new("customer_id", "id")])], null);

        var sql = new PostgreSqlSelectionSqlGenerator().Compile(query);

        Assert.Contains("\"o\".\"customer_tenant_id\" = \"c\".\"tenant_id\" AND \"o\".\"customer_id\" = \"c\".\"id\"", sql.CommandText, StringComparison.Ordinal);
    }

    [Theory]
    [MemberData(nameof(PredicateCases))]
    public void Compile_RendersEachValidatedPredicateAsParameterizedSql(SelectionPredicate predicate, string expectedSql, object[] expectedValues)
    {
    ...
    {
        var table = new PostgreSqlWriteTable(new("sales", "orders"), [new("id", "bigint", NpgsqlTypes.NpgsqlDbType.Bigint, true, false, false, false, null)]);

        Assert.Throws<NotSupportedException>(() => PostgreSqlStableKeyCodec.Encode(new DataPitcher.Core.Identity.StableKey([new("id", 1)]), table));
    }
    ...
    public static IEnumerable<object?[]> PredicateCases()
    {
        ...
        yield return [new ExistsPredicate(Lines, "l", [new(new("o", "tenant_id"), "order_tenant_id"), new(new("o", "id"), "order_id")], null, false), "EXISTS (SELECT 1 FROM \"sales\".\"lines\" AS \"l\" WHERE \"o\".\"tenant_id\" = \"l\".\"order_tenant_id\" AND \"o\".\"id\" = \"l\".\"order_id\")", Array.Empty<object>()];
        yield return [new ExistsPredicate(Lines, "l", [new(new("o", "id"), "order_id")], Comparison(SelectionComparison.Equal, 7, "l"), true), "NOT EXISTS (SELECT 1 FROM \"sales\".\"lines\" AS \"l\" WHERE \"o\".\"id\" = \"l\".\"order_id\" AND \"l\".\"id\" = @p0)", new object[] { 7 }];
    }

    private static readonly TableDefinition Orders = new("sales", "orders", [new("tenant_id", typeof(int), false), new("id", typeof(int), false), new("name", typeof(string), true), new("active", typeof(bool), false), new("day", typeof(DateOnly), false), new("at", typeof(TimeOnly), false), new("occurred", typeof(DateTime), false)], new("pk_orders", ["tenant_id", "id"]), []);
    private static readonly TableDefinition Lines = new("sales", "lines", [new("id", typeof(int), false), new("order_tenant_id", typeof(int), false), new("order_id", typeof(int), false)], new("pk_lines", ["id"]), []);

    private static SelectionQuery Query(SelectionPredicate predicate) => new(new([Orders, Lines], []), new(Orders, "o"), new(Orders.PrimaryKey), [], predicate);
...
public sealed class PostgreSqlTargetCheckpointStoreCoverageTests : IClassFixture<PostgreSqlClosureFixture>
{
    ...
    public async Task InitializeAsync_WhenTheFenceUpdateIsCancelled_ThrowsFenceLost()
    {
        ...
        await Assert.ThrowsAsync<PostgreSqlFenceLostException>(() => store.InitializeAsync(stale with { FenceToken = 2 }, CancellationToken.None));
