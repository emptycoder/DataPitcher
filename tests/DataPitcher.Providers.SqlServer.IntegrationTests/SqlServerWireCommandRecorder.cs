...
public sealed class SqlServerWireCommandRecorder : IAsyncDisposable
{
    ...
    public async Task<int> Count(string tag) =>
        (await SqlTextsAsync()).Count(x => x.Contains(tag, StringComparison.Ordinal));

    public async Task<int> Count(string tag, string table) =>
        (await SqlTextsAsync()).Count(text =>
            text.Contains(tag, StringComparison.Ordinal)
            && text.Contains(SqlServerIdentifier.Quote(table), StringComparison.Ordinal)
        );

    public async Task<bool> AnyContainsLargeInListAsync(int threshold) =>
        (await SqlTextsAsync()).Any(text =>
            text.Split(" IN (", StringSplitOptions.None)
                .Skip(1)
                .Any(part => part.Split(')')[0].Split(',').Length > threshold)
        );
    ...
    {
        await ExecuteAsync(
            $"ALTER EVENT SESSION [{_name}] ON SERVER STATE=STOP; DROP EVENT SESSION [{_name}] ON SERVER;"
        );
    }
}
