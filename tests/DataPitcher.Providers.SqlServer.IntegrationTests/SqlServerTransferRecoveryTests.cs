...
public sealed class SqlServerTransferRecoveryTests(SqlServerClosureFixture fixture)
{
    ...
    public void Build_UsesLexicographicBinaryCollationKeysetSeekingWithoutOffset()
    {
        var query = SqlServerKeysetSeek.Build(
            SqlServerTransferTestData.TextKeyTable(),
    ...
    public void Build_WithACompositeKey_OrsEachPrefixEqualityAndOmitsCollateForNonTextColumns()
    {
        var table = new SqlServerWriteTable(
            new DataPitcher.Core.Plans.TableAddress("dbo", "two_key_rows"),
            [
                new(
                    "region",
                    "nvarchar(64)",
        ...
        Assert.Contains(
            "(s.[region] COLLATE Latin1_General_100_BIN2>@k0 OR s.[region] COLLATE Latin1_General_100_BIN2=@k0 AND s.[id]>@k1)",
            query.Sql,
            StringComparison.Ordinal
        );
        Assert.Contains(
            "ORDER BY s.[region] COLLATE Latin1_General_100_BIN2,s.[id]",
            query.Sql,
            StringComparison.Ordinal
        );
    }
    ...
    {
        var table = new SqlServerWriteTable(
            new DataPitcher.Core.Plans.TableAddress("dbo", "rows"),
            [
                new(
                    "code",
                    "nvarchar(64)",
        ...
        Assert.Throws<InvalidOperationException>(() =>
            SqlServerKeysetSeek.Build(table, new DataPitcher.Core.Identity.StableKey([new("code", "B")]), 10)
        );
    }
    ...
    public async Task ExecuteAsync_WhenWorkerFenceIsStale_RollsBackBusinessRowsDeterministically()
    {
        ...
        var executor = new SqlServerTransferExecutor(
    ...
    public async Task ExecuteAsync_WhenTheCommittedCheckpointIsRemoved_ThrowsWithoutWritingTheMirror()
    {
        ...
        await scope.ExecuteTargetAsync(
            "CREATE TRIGGER [datapitcher].[remove_checkpoint_after_advance] ON [datapitcher].[transfer_checkpoints] AFTER UPDATE AS BEGIN SET NOCOUNT ON; DELETE [datapitcher].[transfer_checkpoints] WHERE job_id IN (SELECT job_id FROM inserted) AND run_id IN (SELECT run_id FROM inserted); END;"
        );
    ...
    public async Task RecoverAsync_WhenTheRunWasNeverInitialized_ThrowsInvalidOperationException()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            executor.RecoverAsync(neverInitialized, SqlServerTransferTestData.Table(), CancellationToken.None)
        );
    }
    ...
    public async Task RecoverAsync_WhenTheManifestHashDiffersFromTheCheckpoint_ThrowsManifestMismatch()
    {
        ...
        await Assert.ThrowsAsync<SqlServerManifestMismatchException>(() =>
            executor.RecoverAsync(resealed, SqlServerTransferTestData.Table(), CancellationToken.None)
        );
    }
