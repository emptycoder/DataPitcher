...
public sealed class SqlServerRunSessionsTests(SqlServerClosureFixture fixture)
{
    ...
    public async Task Sessions_WhenFrozenKeysAreRead_MovesOnlyPlannedRowsAndAdvancesTheCheckpoint()
    {
        ...
        var checkpoint = (
            await target.AcquireFenceReadCheckpointAndJournalAsync(setup.Run, lease, CancellationToken.None)
        ).Checkpoint;
        Assert.Empty(await target.RepairMutationsAsync([], CancellationToken.None));
        await target.QuarantineAsync(
            new TargetMutation("unused", "unused", TargetMutationKind.DisabledTrigger),
            "unused",
            CancellationToken.None
        );
        await using var source = await sessions.OpenKeysetAsync(
            setup.Run,
        ...
        Assert.Equal(
            1,
            (
                await new SqlServerTargetCheckpointStore(scope.TargetConnectionString).ReadAsync(
                    setup.Run.JobId,
                    setup.Run.RunId,
                    CancellationToken.None
                )
    ...
    public async Task Sessions_WhenResumingFromAMidTransferCheckpoint_NeitherSkipsNorDuplicatesRows()
    {
        ...
        {
            var checkpoint = (
                await target.AcquireFenceReadCheckpointAndJournalAsync(
                    setup.Run,
                    Lease(setup.Run.JobId, 1),
                    CancellationToken.None
                )
            ).Checkpoint;
            await using var source = await setup.Sessions.OpenKeysetAsync(
                setup.Run,
                checkpoint.LastStableKey,
                CancellationToken.None,
                checkpoint.LastTable
        ...
        var resumed = (
            await resumedTarget.AcquireFenceReadCheckpointAndJournalAsync(
                setup.Run,
                Lease(setup.Run.JobId, 2),
                CancellationToken.None
            )
        ).Checkpoint;
        await using var resumedSource = await setup.Sessions.OpenKeysetAsync(
            setup.Run,
            resumed.LastStableKey,
            CancellationToken.None,
            resumed.LastTable
        );
    ...
    private static async Task<RunSetup> SetupAsync(SqlServerClosureScope scope)
    {
        await scope.ExecuteAsync(
            "CREATE TABLE dbo.run_rows (id int NOT NULL PRIMARY KEY, code nvarchar(64) NOT NULL); INSERT dbo.run_rows VALUES (1,N'one'),(2,N'two'),(3,N'three'),(4,N'unplanned');"
        );
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.run_rows (id int NOT NULL PRIMARY KEY, code nvarchar(64) NOT NULL);"
        );
        ...
        var target = await profiles.CreateAsync(
            new ConnectionProfileDraft(
                "Target",
                "sqlserver",
                new(SecretReferenceKind.EnvironmentVariable, "target"),
                "dbo",
                "dbo"
            ),
            "target",
        ...
        var schema = await new SqlServerCatalogReader(scope.SourceConnectionString).ReadAsync(
            "dbo",
            CancellationToken.None
        );
        var definition = schema.Table("run_rows").Definition;
        var keys = new Dictionary<DataPitcher.Core.Schema.TableDefinition, DataPitcher.Core.Schema.StableKeySelection>
        {
            [definition] = DataPitcher.Core.Schema.StableKeySelector.Select(definition, null),
        };
        var frozen = new SqlServerStagingTables(
        ...
        var run = await new PlanJobRunCatalog(plans).LoadAsync(
            new TransferJob(jobId, runId, planId, "run-session", JobState.Queued),
            CancellationToken.None
        );
        return new RunSetup(
            control,
            sourceTable,
            run,
            new SqlServerRunSessions(
                plans,
                profiles,
                new Resolver(scope.SourceConnectionString, scope.TargetConnectionString)
            )
        );
    }

    private static TransferPlanContent Plan(TableAddress table, Guid sourceConnectionId, Guid targetConnectionId) =>
        new(
            ...
            [],
            [],
            [new TableConflictPolicy(table, RootConflictPolicy.FailOnConflict)],
            ...
            [
                new PlanTable(
                    new TableMapping(table, table, [new ColumnMapping("id", "id"), new ColumnMapping("code", "code")]),
                    PlanTableState.Root,
                    new ManifestCounts(3, 3, 0, 0),
                    new TopologicalGroup([table]),
    ...
    private static LeaseGrant Lease(Guid jobId, long fenceToken) =>
        new(jobId, "worker", fenceToken, DateTimeOffset.UtcNow.AddMinutes(1), DateTimeOffset.UtcNow);

    private sealed class RunSetup(
        Control control,
        TableAddress sourceTable,
        TransferRun run,
        SqlServerRunSessions sessions
    ) : IDisposable
    {
    ...
    private sealed class Resolver(string source, string target) : ISecretReferenceResolver
    {
        public Task<string> ResolveAsync(SecretReference reference, CancellationToken cancellationToken) =>
            Task.FromResult(StringComparer.Ordinal.Equals(reference.Locator, "source") ? source : target);
    }
    ...
    private sealed class Control : IDisposable
    {
        private readonly string _path = Path.Combine(
            Path.GetTempPath(),
            $"datapitcher-run-sessions-{Guid.NewGuid():N}.db"
        );
