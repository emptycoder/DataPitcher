...
public sealed class SqlServerClosureStoreTests(SqlServerClosureFixture fixture)
{
    ...
    public async Task ExpandAsync_UsesForeignKeyPositionAndProbeReportsTargetTrust()
    {
        ...
        await scope.ExecuteAsync("INSERT dbo.composite_parent VALUES (7,8); INSERT dbo.composite_child VALUES (1,8,7);");
        ...
        Assert.Contains(await store.ExpandAsync(fk, [new StableKey([new("id", 1)])], CancellationToken.None), key => key == new StableKey([new("left_value", 7), new("right_value", 8)]));
        var g = source.Table("untrusted_grandparents").Definition;
        var probe = await store.ProbeTargetAsync(p, [new ClosureRelationship(source.ForeignKey("FK_P_G"))], [new StableKey([new("id", 2)])], CancellationToken.None);
