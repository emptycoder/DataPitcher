...
public sealed class SqlServerTargetCheckpointStoreTests(SqlServerClosureFixture fixture)
{
    ...
    public async Task AdvanceAsync_RecordsTheBatchAndRejectsASupersededFence()
    {
        ...
        await store.AdvanceAsync(connection, transaction, old, SqlServerTransferTestData.Table(), SqlServerTransferTestData.Batch(0, (1, "one")), 1, 1, 0, CancellationToken.None);
        ...
        await Assert.ThrowsAsync<SqlServerFenceLostException>(() => store.AdvanceAsync(connection, stale, old, SqlServerTransferTestData.Table(), SqlServerTransferTestData.Batch(1, (2, "two")), 1, 1, 0, CancellationToken.None));
    ...
    public async Task InitializeAsync_WhenManifestHashDiffersFromTheStoredCheckpoint_ThrowsWithoutChangingIt()
    {
        ...
        await Assert.ThrowsAsync<SqlServerManifestMismatchException>(() => store.InitializeAsync(resealed, CancellationToken.None));
    ...
    public async Task InitializeAsync_WhenAnOlderFenceArrivesAfterANewerOne_ThrowsFenceLost()
    {
        ...
        await Assert.ThrowsAsync<SqlServerFenceLostException>(() => store.InitializeAsync(older, CancellationToken.None));
    }
    ...
    public async Task InitializeAsync_WhenTheCheckpointFenceUpdateDoesNotAdvance_ThrowsFenceLost()
    {
        ...
        var older = SqlServerTransferTestData.Context();
        await store.InitializeAsync(older, CancellationToken.None);
        await scope.ExecuteTargetAsync("CREATE TRIGGER [datapitcher].[suppress_checkpoint_fence] ON [datapitcher].[transfer_checkpoints] INSTEAD OF UPDATE AS RETURN;");
