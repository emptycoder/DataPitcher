...
public sealed class SqlServerBatchExecutionTests(SqlServerClosureFixture fixture)
{
    ...
    public async Task StageAsync_WhenTheSecondNativeCopyFails_RollbackLeavesNoBusinessRowOrCheckpointAdvance()
    {
        ...
        await scope.ExecuteTargetAsync("CREATE TABLE dbo.transfer_rows (id int NOT NULL PRIMARY KEY, code nvarchar(2) NOT NULL);");
        var context = SqlServerTransferTestData.Context();
        var table = new SqlServerWriteTable(SqlServerTransferTestData.Table().Target, [
            new("id", "int", typeof(int), SqlDbType.Int, true, false, false, false, false, null),
            new("code", "nvarchar(2)", typeof(string), SqlDbType.NVarChar, false, false, false, false, false, "Latin1_General_100_BIN2")
        ]);
        ...
        await writer.StageAsync(connection, transaction, context, table, SqlServerTransferTestData.Batch(0, (1, "ok")), CancellationToken.None);
        await Assert.ThrowsAnyAsync<Exception>(() => writer.StageAsync(connection, transaction, context, table, SqlServerTransferTestData.Batch(1, (2, "too-long")), CancellationToken.None));
    ...
    public async Task ApplyAsync_UsesSeparateStatementsAndRecordsOnlyCommittedAffectedKeys(SqlServerConflictPolicy policy, int affected, int updates, int inserts)
    {
        ...
        await connection.OpenAsync();
        await using var transaction = (SqlTransaction)await connection.BeginTransactionAsync(CancellationToken.None);
        await new SqlServerBatchStageWriter().StageAsync(connection, transaction, context, SqlServerTransferTestData.Table(), batch, CancellationToken.None);
        var result = await new SqlServerBatchApplier().ApplyAsync(connection, transaction, context, SqlServerTransferTestData.Table(), batch, CancellationToken.None);
    ...
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync("CREATE TABLE dbo.transfer_rows (id int NOT NULL PRIMARY KEY, code nvarchar(64) NOT NULL, note nvarchar(64) NULL);");
        var table = new SqlServerWriteTable(new TableAddress("dbo", "transfer_rows"), [
            new("id", "int", typeof(int), SqlDbType.Int, true, false, false, false, false, null),
            new("code", "nvarchar(64)", typeof(string), SqlDbType.NVarChar, false, false, false, false, false, "Latin1_General_100_BIN2"),
            new("note", "nvarchar(64)", typeof(string), SqlDbType.NVarChar, false, false, false, false, true, "Latin1_General_100_BIN2")
        ]);
        var context = SqlServerTransferTestData.Context();
        var batch = new SqlServerTransferBatch(0, [new SqlServerTransferRow(new StableKey([new KeyComponent("id", 1)]), new Dictionary<string, object?> { ["id"] = 1, ["code"] = "ok", ["note"] = null })], new StableKey([new KeyComponent("id", 1)]), SqlServerConflictPolicy.InsertOnly);

        await using var connection = new SqlConnection(scope.TargetConnectionString);
        await connection.OpenAsync();
        await using var transaction = (SqlTransaction)await connection.BeginTransactionAsync(CancellationToken.None);
        await new SqlServerBatchStageWriter().StageAsync(connection, transaction, context, table, batch, CancellationToken.None);
        await new SqlServerBatchApplier().ApplyAsync(connection, transaction, context, table, batch, CancellationToken.None);
        await transaction.CommitAsync(CancellationToken.None);

        Assert.True(await scope.ScalarTargetAsync<bool>("SELECT CAST(CASE WHEN note IS NULL THEN 1 ELSE 0 END AS bit) FROM dbo.transfer_rows WHERE id=1"));
    }
    ...
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync("CREATE TABLE dbo.identity_rows (id int IDENTITY PRIMARY KEY, code nvarchar(64) NOT NULL);");
        var table = new SqlServerWriteTable(new TableAddress("dbo", "identity_rows"), [
            new("id", "int", typeof(int), SqlDbType.Int, true, true, false, false, false, null),
            new("code", "nvarchar(64)", typeof(string), SqlDbType.NVarChar, false, false, false, false, false, "Latin1_General_100_BIN2")
        ]);
        var context = SqlServerTransferTestData.Context();
        var batch = new SqlServerTransferBatch(0, [new SqlServerTransferRow(new StableKey([new KeyComponent("id", 100)]), new Dictionary<string, object?> { ["id"] = 100, ["code"] = "explicit" })], new StableKey([new KeyComponent("id", 100)]), SqlServerConflictPolicy.InsertOnly);

        await using var connection = new SqlConnection(scope.TargetConnectionString);
        await connection.OpenAsync();
        await using var transaction = (SqlTransaction)await connection.BeginTransactionAsync(CancellationToken.None);
        await new SqlServerBatchStageWriter().StageAsync(connection, transaction, context, table, batch, CancellationToken.None);
        var result = await new SqlServerBatchApplier().ApplyAsync(connection, transaction, context, table, batch, CancellationToken.None);
        await transaction.CommitAsync(CancellationToken.None);
    ...
    public async Task ApplyAsync_WhenUpsertHasNoMutableColumnsBesidesTheKey_SkipsTheUpdateStatement()
    {
        ...
        var table = new SqlServerWriteTable(new TableAddress("dbo", "key_only_rows"), [new("id", "int", typeof(int), SqlDbType.Int, true, false, false, false, false, null)]);
        await scope.ExecuteTargetAsync("INSERT dbo.key_only_rows VALUES (1);");
        var context = SqlServerTransferTestData.Context();
        var batch = new SqlServerTransferBatch(0, [new SqlServerTransferRow(new StableKey([new KeyComponent("id", 1)]), new Dictionary<string, object?> { ["id"] = 1 })], new StableKey([new KeyComponent("id", 1)]), SqlServerConflictPolicy.Upsert);
