...
public sealed class SqlServerStrictExactTests(SqlServerClosureFixture fixture)
{
    ...
    public async Task EnsureAvailableAsync_WhenThePlannedTargetHasAnEnabledTrigger_RefusesStrictExact()
    {
        ...
        await scope.ExecuteTargetAsync(
            "CREATE TRIGGER dbo.transfer_trigger ON dbo.transfer_rows AFTER INSERT AS SELECT 1;"
        );
        await Assert.ThrowsAsync<SqlServerStrictExactBlockedException>(() =>
            new SqlServerStrictExact(scope.TargetConnectionString).EnsureAvailableAsync(
    ...
    public async Task EnsureAvailableAsync_WhenThePlannedTargetHasAnInboundCascade_RefusesStrictExact()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.transfer_rows (id int PRIMARY KEY,code nvarchar(64)); CREATE TABLE dbo.transfer_children (id int PRIMARY KEY,parent_id int REFERENCES dbo.transfer_rows(id) ON UPDATE CASCADE);"
        );
        await Assert.ThrowsAsync<SqlServerStrictExactBlockedException>(() =>
            new SqlServerStrictExact(scope.TargetConnectionString).EnsureAvailableAsync(
                SqlServerTransferTestData.Table(),
    ...
    public async Task EnsureAvailableAsync_WhenTargetHasNoSideEffects_AllowsStrictExact()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync("CREATE TABLE dbo.transfer_rows (id int PRIMARY KEY,code nvarchar(64));");
        await new SqlServerStrictExact(scope.TargetConnectionString).EnsureAvailableAsync(
            SqlServerTransferTestData.Table(),
            CancellationToken.None
        );
    }
    ...
    public async Task RealignAsync_DiscoversTheIdentityAndAdvancesPastExplicitKeys()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.sequence_rows (id bigint IDENTITY(1,1) PRIMARY KEY, code nvarchar(64) NOT NULL); SET IDENTITY_INSERT dbo.sequence_rows ON; INSERT dbo.sequence_rows (id,code) VALUES (10,N'ten'); SET IDENTITY_INSERT dbo.sequence_rows OFF;"
        );
        await new SqlServerIdentityRealigner(scope.TargetConnectionString).RealignAsync(
            SequenceTable(),
            "id",
            CancellationToken.None
        );
        Assert.Equal(
            11L,
            await scope.ScalarTargetAsync<long>("INSERT dbo.sequence_rows (code) OUTPUT INSERTED.id VALUES (N'next')")
        );
    }
    ...
    public async Task RealignAsync_WhenThePositiveIdentityIsAlreadyAhead_DoesNotRewindIt()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.sequence_rows (id bigint IDENTITY(1,1) PRIMARY KEY, code nvarchar(64) NOT NULL); SET IDENTITY_INSERT dbo.sequence_rows ON; INSERT dbo.sequence_rows (id,code) VALUES (10,N'ten'); SET IDENTITY_INSERT dbo.sequence_rows OFF; DBCC CHECKIDENT ('dbo.sequence_rows', RESEED, 50);"
        );
        await new SqlServerIdentityRealigner(scope.TargetConnectionString).RealignAsync(
            SequenceTable(),
            "id",
        ...
        Assert.Equal(
            51L,
            await scope.ScalarTargetAsync<long>("INSERT dbo.sequence_rows (code) OUTPUT INSERTED.id VALUES (N'ahead')")
        );
    }
    ...
    public async Task RealignAsync_WhenThePositiveIdentityIsBehindTheOccupiedMaximum_ReseedsIt()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.sequence_rows (id bigint IDENTITY(1,1) PRIMARY KEY, code nvarchar(64) NOT NULL); SET IDENTITY_INSERT dbo.sequence_rows ON; INSERT dbo.sequence_rows (id,code) VALUES (10,N'ten'); SET IDENTITY_INSERT dbo.sequence_rows OFF; DBCC CHECKIDENT ('dbo.sequence_rows', RESEED, 0);"
        );
        await new SqlServerIdentityRealigner(scope.TargetConnectionString).RealignAsync(
            SequenceTable(),
            "id",
        ...
        Assert.Equal(
            ...
            await scope.ScalarTargetAsync<long>(
                "INSERT dbo.sequence_rows (code) OUTPUT INSERTED.id VALUES (N'reseeded')"
            )
        );
    }
    ...
    public async Task RealignAsync_WhenAnUnissuedPositiveIdentityIsAlreadyAhead_DoesNotRewindIt()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.sequence_rows (id bigint IDENTITY(100,1) PRIMARY KEY, code nvarchar(64) NOT NULL); SET IDENTITY_INSERT dbo.sequence_rows ON; INSERT dbo.sequence_rows (id,code) VALUES (10,N'ten'); SET IDENTITY_INSERT dbo.sequence_rows OFF;"
        );
        await new SqlServerIdentityRealigner(scope.TargetConnectionString).RealignAsync(
            SequenceTable(),
            "id",
    ...
    public async Task RealignAsync_WhenTheColumnIsNotAnIdentity_LeavesItsExistingValuesAlone()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.sequence_rows (id bigint PRIMARY KEY, code nvarchar(64) NOT NULL); INSERT dbo.sequence_rows VALUES (10,N'ten');"
        );
        await new SqlServerIdentityRealigner(scope.TargetConnectionString).RealignAsync(
            SequenceTable(),
            "id",
    ...
    public async Task RealignAsync_WhenTheIdentityTableIsEmpty_DoesNotAdvanceTheGenerator()
    {
        await using var scope = await fixture.CreateScopeAsync();
        await scope.ExecuteTargetAsync(
            "CREATE TABLE dbo.sequence_rows (id bigint IDENTITY(1,1) PRIMARY KEY, code nvarchar(64) NOT NULL);"
        );
        await new SqlServerIdentityRealigner(scope.TargetConnectionString).RealignAsync(
            SequenceTable(),
            "id",
        ...
        Assert.Equal(
            1L,
            await scope.ScalarTargetAsync<long>("INSERT dbo.sequence_rows (code) OUTPUT INSERTED.id VALUES (N'first')")
        );
    }

    private static SqlServerWriteTable SequenceTable() =>
        new(
            new DataPitcher.Core.Plans.TableAddress("dbo", "sequence_rows"),
            [
