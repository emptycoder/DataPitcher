...
public sealed class SqlServerStagingTablesTests(SqlServerClosureFixture fixture)
{
    ...
    public async Task Stages_UseNativeKeyTypesCompleteUniqueIndexAndFirstGeneration()
    {
        ...
        await using var first = new SqlServerStagingTables(
            scope.SourceConnectionString,
            scope.TargetConnectionString,
            schema,
            keys
        );
        await using var second = new SqlServerStagingTables(
            scope.SourceConnectionString,
            scope.TargetConnectionString,
            schema,
            keys
    ...
    public async Task Stages_WhenPlanIsSpecified_UseDeterministicNamesAndRemainAfterDisposal()
    {
        ...
        var first = new SqlServerStagingTables(
            scope.SourceConnectionString,
            scope.TargetConnectionString,
            schema,
            keys,
        ...
        var second = new SqlServerStagingTables(
            scope.SourceConnectionString,
            scope.TargetConnectionString,
            schema,
            keys,
        ...
        var sourceName = first.SourceTableName(table);
        var inputName = first.InputTableName(table);
        ...
        Assert.Equal(
            ...
            await scope.ScalarAsync<int>(
                "SELECT CASE WHEN OBJECT_ID(N'"
                    + SqlServerStagingTables.Qualified(sourceName)
                    + "',N'U') IS NULL THEN 0 ELSE 1 END"
            )
        );
        await scope.ExecuteAsync(
            "DROP TABLE "
                + SqlServerStagingTables.Qualified(inputName)
                + "; DROP TABLE "
                + SqlServerStagingTables.Qualified(sourceName)
        );
    }
