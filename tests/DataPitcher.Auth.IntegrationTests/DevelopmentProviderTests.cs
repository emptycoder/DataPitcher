...
public sealed class DevelopmentProviderTests
{
    ...
    public void HostingExtensions_WhenNoProvidersAreEnabled_RejectsComposition()
    {
        ...
        var exception = Assert.Throws<InvalidOperationException>(() =>
            services.AddDataPitcherAuthenticationProviders(
                configuration,
                new TestEnvironment { EnvironmentName = "Test" }
            )
        );
    ...
    public void HostingExtensions_WhenDevelopmentIsEnabled_RegistersItsNormalizer()
    {
        ...
        var normalized = normalizer.Normalize(
            new ClaimsPrincipal(
                new ClaimsIdentity(
                    new[] { new Claim("sub", "subject"), new Claim("roles", "Planner"), new Claim("groups", "group-a") }
                )
            ),
            "https://development.test"
        );
    ...
    public void HostingExtensions_WhenGenericIsEnabled_RegistersItsNormalizer()
    {
        ...
        var normalized = normalizer.Normalize(
            new ClaimsPrincipal(new ClaimsIdentity(new[] { new Claim("sub", "subject") })),
            "https://issuer.test"
        );
    ...
    public void DevelopmentNormalizer_RejectsAValidatedTokenWithoutSubject()
    {
        ...
        var normalizer = provider.GetRequiredKeyedService<IExternalPrincipalNormalizer>("development");
        var exception = Assert.Throws<InvalidOperationException>(() =>
            normalizer.Normalize(new ClaimsPrincipal(new ClaimsIdentity()), "https://development.test")
        );
    ...
    public void HostingExtensions_WhenDevelopmentConfigurationIsMissing_RejectsItsEmptySigningKey()
    {
        var services = new ServiceCollection();
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { ["Authentication:Development:Enabled"] = "true" })
            .Build();
        var exception = Assert.Throws<InvalidOperationException>(() =>
            services.AddDataPitcherAuthenticationProviders(
                configuration,
    ...
    public void HostingExtensions_WhenDevelopmentIsEnabledInRelease_RejectsConfiguration()
    {
        ...
        var configuration = new ConfigurationBuilder()
            ...
            .Build();
        var exception = Assert.Throws<InvalidOperationException>(() =>
            services.AddDataPitcherAuthenticationProviders(configuration, new TestEnvironment())
        );
        Assert.Equal("Development authentication cannot be enabled in a Release artifact.", exception.Message);
    }
#endif

    private static DevelopmentProviderOptions Options(string signingKey = "01234567890123456789012345678901") =>
        new()
        {
            SchemeName = "development",
            ProviderInstance = "development",
            Issuer = "https://development.test",
            Audience = "api",
