...
public sealed class EntraProviderTests
{
    ...
    {
        var normalizer = new EntraPrincipalNormalizer(new EntraProviderOptions { SchemeName = "entra", ProviderInstance = "entra-prod", Instance = "https://login.test/", TenantId = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", ClientId = "client" });
        Assert.Throws<InvalidOperationException>(() => normalizer.Normalize(Principal(new Claim("idtyp", "unknown")), "https://login.test/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/v2.0"));
    }
    ...
    {
        var normalizer = new EntraPrincipalNormalizer(new EntraProviderOptions { SchemeName = "entra", ProviderInstance = "entra-prod", Instance = "https://login.test/", TenantId = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", ClientId = "client" });
        Assert.Throws<InvalidOperationException>(() => normalizer.Normalize(new ClaimsPrincipal(new ClaimsIdentity(new[] { new Claim("oid", "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb") })), "https://login.test/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/v2.0"));
        Assert.Throws<InvalidOperationException>(() => normalizer.Normalize(new ClaimsPrincipal(new ClaimsIdentity(new[] { new Claim("tid", "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa") })), "https://login.test/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/v2.0"));
    }

    [Fact]
    public void EntraProviderRegistration_ValidatesRequiredFieldsAndTenantConfiguration()
    {
        Assert.Throws<ArgumentException>(() => new EntraProviderRegistration(Section(new() { ["SchemeName"] = "", ["ProviderInstance"] = "entra-prod", ["Instance"] = "https://login.test/", ["TenantId"] = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", ["ClientId"] = "client" })));
        Assert.Throws<ArgumentException>(() => new EntraProviderRegistration(Section(new() { ["SchemeName"] = "entra", ["ProviderInstance"] = "entra-prod", ["Instance"] = "https://login.test/", ["TenantId"] = "not-a-guid", ["ClientId"] = "client" })));
        Assert.Throws<ArgumentException>(() => new EntraProviderRegistration(Section(new() { ["SchemeName"] = "entra", ["ProviderInstance"] = "entra-prod", ["Instance"] = "https://login.test/", ["TenantId"] = "organizations", ["ClientId"] = "client" })));
        Assert.Throws<ArgumentException>(() => new EntraProviderRegistration(Section(new() { ["SchemeName"] = "entra", ["ProviderInstance"] = "entra-prod", ["Instance"] = "https://login.test/", ["TenantId"] = "organizations", ["ClientId"] = "client", ["AllowedTenantIds:0"] = "not-a-guid" })));
