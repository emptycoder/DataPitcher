...
{
    [Fact]
    public void Core_ReferencesNoAspNetDataAccessOrProviderPackage()
    {
        var names = References(Project("DataPitcher.Core")).Concat(Packages(Project("DataPitcher.Core")));
        Assert.DoesNotContain(
            names,
            name =>
                name.StartsWith("Microsoft.AspNetCore", StringComparison.Ordinal)
                || name.StartsWith("DataPitcher.Providers.", StringComparison.Ordinal)
                || name
                    is "Dapper"
                        or "LinqToDB"
    ...
    [Fact]
    public void OnlyApi_ReferencesConcreteProviderProjects() =>
        Assert.DoesNotContain(
            Projects()
                .Where(p =>
                    Path.GetRelativePath(Root, p)
    ...
    public void AuthHosting_IsTheOnlySourceProjectReferencingConcreteAuthenticationProviders()
    {
        var concrete = new[]
        {
            "DataPitcher.Auth.Entra",
            "DataPitcher.Auth.OpenIdConnect",
            "DataPitcher.Auth.Development",
        };
        var sourceReferences = Projects()
            .Where(project =>
                Path.GetRelativePath(Root, project)
                    .StartsWith("src" + Path.DirectorySeparatorChar, StringComparison.Ordinal)
            )
            .ToDictionary(Name, References);
        Assert.Equal(
            concrete.OrderBy(name => name, StringComparer.Ordinal),
            sourceReferences["DataPitcher.Auth.Hosting"]
                .Where(concrete.Contains)
                .OrderBy(name => name, StringComparer.Ordinal)
        );
        Assert.DoesNotContain(
            sourceReferences.Where(pair => pair.Key != "DataPitcher.Auth.Hosting").SelectMany(pair => pair.Value),
            concrete.Contains
        );
    }
    ...
    public void Api_IsCompositionRootAndCoreRemainsDependencyFree()
    {
        ...
        Assert.Equal(
            ...
            References(api).OrderBy(name => name, StringComparer.Ordinal)
        );
        Assert.DoesNotContain(
            Projects()
                .Where(p =>
    ...
    private static IEnumerable<string> Projects(string directory)
    {
        if (
            directory != Root
            && (
                Path.GetFileName(directory) == ".worktrees"
    ...
    private static IEnumerable<string> Packages(string project) =>
        XDocument.Load(project).Descendants("PackageReference").Select(x => x.Attribute("Include")!.Value);
    ...
    private static string FindRoot()
    {
        for (
            ...
            if (File.Exists(Path.Combine(directory.FullName, "DataPitcher.sln")))
                return directory.FullName;
        throw new DirectoryNotFoundException("DataPitcher.sln");
    }
}
