...
public sealed class JobWorkerTests
{
    ...
    {
        var worker = new JobWorker(null!, null!, null!, null!, null!, null!, null!, null!, null!, "worker-a", TimeSpan.FromMinutes(1), TimeSpan.FromMinutes(1));
    ...
    public async Task JobWorker_WhenNoJobIsQueued_WaitsForTheNextPollInterval()
    {
        ...
        var worker = new JobWorker(new NeverClaimsJobControl(), null!, null!, null!, null!, null!, null!, delay, fixture.Clock, "worker-a", TimeSpan.FromMinutes(1), pollInterval);
    ...
    public async Task JobWorker_WhenClaimRunsNormally_MirrorsTheTargetCheckpointAndDisposesItsSessions()
    {
        ...
        var calls = new List<string>(); var jobs = new SingleClaimJobControl(new JobClaim(job, lease, false), calls); var source = new TestTransferReadSession(unit, "source-connection"); var target = new TestTargetRunSession(checkpoint, "target-connection", calls); var mirror = new RecordingCheckpointMirror(calls);
        var worker = new JobWorker(jobs, new TestJobRunCatalog(run), new TestTargetRunSessionFactory(target), new TestTransferReadSessionFactory(source), new LeaseRenewer(new LeaseStore(fixture.Database, fixture.Clock), new BlockingWorkerDelay()), mirror, new NoWorkerFaults(), new BlockingWorkerDelay(), fixture.Clock, "worker-a", ttl, TimeSpan.FromMinutes(1));

        await worker.StartAsync(CancellationToken.None);
        await jobs.MarkedVerifying;
        await worker.StopAsync(CancellationToken.None);

        Assert.Equal(["Prepare", "Running", "Apply", "Mirror", "Verifying"], calls); Assert.Single(mirror.Checkpoints, item => item == checkpoint); Assert.True(source.Disposed); Assert.True(target.Disposed); Assert.NotEqual(source.ConnectionOwnerId, target.ConnectionOwnerId);
    }
}
