...
{
    private readonly TaskCompletionSource<JobState> _terminalState = new(TaskCreationOptions.RunContinuationsAsynchronously);
    private readonly TaskCompletionSource<bool> _stopped = new(TaskCreationOptions.RunContinuationsAsynchronously);
    private int _available = 1;
    ...
    public Task MarkRunningAsync(LeaseGrant lease, CancellationToken cancellationToken) { State = JobState.Running; return Task.CompletedTask; }
...
internal sealed class PrefetchedReadSession(TransferUnit first, TransferUnit prefetched) : ITransferReadSession
{
    ...
    public Task DiscardUncommittedAsync(CancellationToken cancellationToken) { Discarded = true; DiscardToken = cancellationToken; _next = null; _prefetched = null; return Task.CompletedTask; }
    public ValueTask DisposeAsync() { Disposed = true; return ValueTask.CompletedTask; }
}
...
internal sealed class CommitBarrierTargetSession(TargetCheckpoint initialCheckpoint) : ITargetRunSession
{
    ...
    private readonly TaskCompletionSource<bool> _releaseCommit = new(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    public Task<RecoverySnapshot> AcquireFenceReadCheckpointAndJournalAsync(TransferRun run, LeaseGrant lease, CancellationToken cancellationToken) => Task.FromResult(new RecoverySnapshot(initialCheckpoint, []));
    ...
    public async Task<TargetCheckpoint> ApplyAsync(TransferRun run, LeaseGrant lease, TransferUnit unit, CancellationToken cancellationToken)
    {
        ...
        if (DurableBatchSequences.Count == 1) _firstCommit.TrySetResult(true);
        return new TargetCheckpoint(run.JobId, run.RunId, unit.BatchSequence, unit.LastStableKey, unit.RowCount, run.ManifestSealHash, lease.FenceToken);
    }
    public Task DiscardUncommittedAsync(CancellationToken cancellationToken) { Discarded = true; DiscardToken = cancellationToken; return Task.CompletedTask; }
