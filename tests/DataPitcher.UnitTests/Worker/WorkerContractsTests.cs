...
public sealed class WorkerContractsTests
{
    ...
    public async Task Faults_WhenPointIsConfigured_ThrowsOnlyForThatPoint()
    {
        ...
        await Assert.ThrowsAsync<SimulatedWorkerFaultException>(() =>
            faults.HitAsync(TransferFaultPoint.BeforeTargetCommit, CancellationToken.None)
        );
    ...
    public void TransferProgressContracts_WhenBatchIsCommitted_PreserveByteCounts()
    {
        var unit = new TransferUnit(
            6,
            new StableKey([new KeyComponent("Id", 12)]),
            6,
    ...
    public void TransferRun_WhenSealed_PreservesItsIdentityAndResumeCapability()
    {
        ...
        Assert.NotEqual(Guid.Empty, run.JobId);
        Assert.NotEqual(Guid.Empty, run.RunId);
        Assert.Equal("seal", run.ManifestSealHash);
        Assert.True(run.SupportsDurableResume);
    }

    [Fact]
    public void TransferContracts_WhenPlanAndTableAreProvided_PreserveThem()
    {
        var table = new TableAddress("dbo", "Orders");
        var row = new TransferRow([1], 1);
        var rows = new[] { row };
    ...
    public void RecoverySnapshot_WhenCreated_PreservesTheTargetCheckpointAndMutationJournal()
    {
        ...
        var entry = new MutationJournalEntry(
            Guid.NewGuid(),
            mutation,
            MutationJournalState.PendingRepair,
            "repair required"
        );
        var snapshot = new RecoverySnapshot(checkpoint, [entry]);

        Assert.Same(checkpoint, snapshot.Checkpoint);
        Assert.Single(snapshot.Mutations, item => item.EntryId == entry.EntryId);
        Assert.Equal(mutation, snapshot.Mutations[0].Mutation);
        Assert.Equal(MutationJournalState.PendingRepair, snapshot.Mutations[0].State);
        Assert.Equal("repair required", snapshot.Mutations[0].Detail);
    }
    ...
    public void TransferAttemptException_WhenCreated_PreservesCommitDispositionAndCause()
    {
        ...
        Assert.Equal(CommitDisposition.Unknown, exception.Disposition);
        Assert.Same(cause, exception.InnerException);
        Assert.Equal(cause.Message, exception.Message);
    }
    ...
    private sealed class ScriptedWorkerFaults(TransferFaultPoint point) : IWorkerFaults
    {
        ...
        public Task HitAsync(TransferFaultPoint current, CancellationToken cancellationToken)
        {
            ...
            if (_pending && current == point)
            {
                _pending = false;
                throw new SimulatedWorkerFaultException(point);
            }
