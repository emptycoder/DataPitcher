...
{
    [Fact]
    public void TarjanScc_WhenGraphIsAcyclic_EachNodeIsItsOwnComponent()
    {
        var (g, a, b, _) = G(("A", "B"), ("B", "C"));
        Assert.All(TarjanScc.Find(g), c => Assert.Single(c.Tables));
    }

    [Fact]
    public void TarjanScc_WhenTwoTablesFormCycle_ProducesSingleComponentOfSizeTwo()
    {
        var (g, a, b, _) = G(("A", "B"), ("B", "A"));
        Assert.Contains(TarjanScc.Find(g), c => c.Tables.Count == 2 && c.Tables.Contains(a) && c.Tables.Contains(b));
    }

    [Fact]
    public void TarjanScc_WhenNodeHasOnlySelfEdge_DoesNotCreateMultiNodeComponent()
    {
        var (g, _, _, _) = G(("A", "A"));
        Assert.Single(Assert.Single(TarjanScc.Find(g)).Tables);
    }
    ...
    public void CondensedGraph_IsAlwaysAcyclic()
    {
        ...
        Assert.True(
            condensed.Components.Count < g.Tables.Count,
            "Precondition: the A/B cycle must have genuinely collapsed into one component."
        );
        Assert.True(condensed.IsAcyclic());
    }

    [Fact]
    public void CondensedGraph_TopologicalLayers_RespectEveryEdge()
    {
        var (g, _, _, _) = G(("A", "B"), ("B", "C"));
        var condensed = new CondensedGraph(g);
        var layer = condensed
            .TopologicalLayers()
            .SelectMany((x, i) => x.Select(id => (id, i)))
            .ToDictionary(x => x.id, x => x.i);
        Assert.All(condensed.Edges, edge => Assert.True(layer[edge.Parent] < layer[edge.Child]));
    }

    [Fact]
    public void SccAndCondensedGraph_WhenInputOrReturnedViewsAreMutated_RemainImmutable()
    {
        var tables = new List<TableDefinition> { new("dbo", "A", [], null, []) };
        var scc = new Scc(1, tables);
        tables.Clear();
        Assert.Single(scc.Tables);
        Assert.Throws<NotSupportedException>(() => ((IList<TableDefinition>)scc.Tables).RemoveAt(0));
        var (graph, _, _, _) = G(("A", "B"));
        ...
        Assert.Throws<NotSupportedException>(() =>
            ((IList<TableDefinition>)scc.Tables)[0] = new TableDefinition("dbo", "Z", [], null, [])
        );
        Assert.Throws<NotSupportedException>(() => ((IList<Scc>)condensed.Components)[0] = new Scc(99, []));
        Assert.Throws<NotSupportedException>(() =>
            ((IList<CondensedEdge>)condensed.Edges)[0] = new CondensedEdge(0, 0)
        );
    ...
    public void CondensedGraph_TopologicalLayers_PlaceParentsBeforeChildren()
    {
        ...
        Assert.True(
            LayerOf(customers) < LayerOf(orders),
            "Customers (parent) must be in a strictly earlier transfer layer than Orders (child)."
        );
    }
    ...
    private static string Render(CondensedGraph graph) =>
        string.Join(
            ";",
            graph.Components.Select(c => $"{c.Id}:[{string.Join(",", c.Tables.Select(t => $"{t.Schema}.{t.Name}"))}]")
        )
        + "|"
    ...
    private static string RenderFull(DependencyGraph graph)
    {
        ...
        var depsPart = string.Join(
            ";",
            graph.Tables.Select(t =>
                $"{Q(t)}=>[{string.Join(",", graph.DependenciesOf(t).Select(fk => fk.Name + ":" + Q(fk.ParentTable)))}]"
            )
        );
        var dependentsPart = string.Join(
            ";",
            graph.Tables.Select(t =>
                $"{Q(t)}<=[{string.Join(",", graph.DependentsOf(t).Select(fk => fk.Name + ":" + Q(fk.ChildTable)))}]"
            )
        );
        var componentsPart = string.Join(
            ";",
            condensed.Components.Select(c => $"{c.Id}:[{string.Join(",", c.Tables.Select(Q))}]")
        );
    ...
    private static (DependencyGraph Graph, TableDefinition A, TableDefinition B, TableDefinition C) G(
        params (string Child, string Parent)[] edges
    )
    {
        var names = edges.SelectMany(x => new[] { x.Child, x.Parent }).Distinct().ToArray();
        var tables = names.ToDictionary(x => x, x => new TableDefinition("dbo", x, [], null, []));
        return (
            new DependencyGraph(
                tables.Values,
                edges.Select(
            ...
            tables.GetValueOrDefault("A")!,
            tables.GetValueOrDefault("B")!,
            tables.GetValueOrDefault("C")!
        );
    }
}
