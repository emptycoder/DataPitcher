...
public sealed class RowGraphModelsTests
{
    ...
    public void RowGraphRequest_WhenInputsAreMutated_ExposesImmutableRowsAndReferences()
    {
        ...
        var fk = ForeignKey(employees, employees);
        var manager = Row(employees, 1);
        var employee = Row(employees, 2);
        ...
        rows.Clear();
        references.Clear();
        Assert.Equal(2, request.PlannedRows.Count);
    ...
    public void RowGraphAnalysis_WhenExternalParentIsMissing_ReportsItSeparatelyFromAcyclicOrder()
    {
        ...
        var fk = ForeignKey(employees, employees);
        var employee = Row(employees, 2);
        var absentManager = Row(employees, 99);
        var analysis = new RowGraphAnalysis([employee], [], [new(employee, fk, absentManager)]);
        Assert.True(analysis.IsAcyclic);
        Assert.Empty(analysis.UnreachedRows);
    ...
    private static ForeignKeyDefinition ForeignKey(TableDefinition child, TableDefinition parent) =>
        new($"FK_{child.Name}_{parent.Name}", child, parent, ["ManagerId"], ["Id"], true, true);
