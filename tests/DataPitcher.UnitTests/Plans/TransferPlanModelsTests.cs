...
public sealed class TransferPlanModelsTests
{
    ...
    public void TransferPlanContent_WhenSourceCollectionsChange_RetainsIndependentValues()
    {
        ...
        var table = new PlanTable(
            new(PlanTestData.Customers, PlanTestData.Customers, columns),
            PlanTableState.Root,
            new(1, 1, 0, 0),
            new(group),
        ...
        columns[0] = new("Id", "Changed");
        group[0] = PlanTestData.Orders;
        tables[0] = PlanTestData.Table(PlanTestData.Orders, PlanTableState.Conflict);
    ...
    public void TransferPlanContent_WhenExposedCollectionsAreDowncast_RejectsIndexerAssignment()
    {
        ...
        Assert.Throws<NotSupportedException>(() =>
            ((IList<ColumnMapping>)table.Mapping.Columns)[0] = new("Id", "Changed")
        );
        Assert.Throws<NotSupportedException>(() =>
            ((IList<TableAddress>)table.TopologicalGroup.Tables)[0] = PlanTestData.Orders
        );
        Assert.Throws<NotSupportedException>(() =>
            ((IList<SelectionReference>)content.Selections)[0] = content.Selections[0]
        );
    }
    ...
    [Theory]
    [InlineData(PlanTableState.Root)]
    [InlineData(PlanTableState.RequiredDependency)]
    [InlineData(PlanTableState.ExplicitDependent)]
    [InlineData(PlanTableState.TargetSatisfied)]
    [InlineData(PlanTableState.Excluded)]
    [InlineData(PlanTableState.Blocked)]
    [InlineData(PlanTableState.Conflict)]
    [InlineData(PlanTableState.CycleMember)]
    public void PlanTable_RecordsEveryDefinedState(PlanTableState state) =>
        Assert.Equal(state, PlanTestData.Table(PlanTestData.Orders, state).State);
}
