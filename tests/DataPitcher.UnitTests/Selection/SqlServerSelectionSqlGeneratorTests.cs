...
public sealed class SqlServerSelectionSqlGeneratorTests
{
    ...
    public void Compile_ProjectsCompositeKeysInDeclarationOrder()
    {
        var orders = new TableDefinition(
            "dbo",
            "orders",
            [new("tenant_id", typeof(int), false), new("order_id", typeof(int), false)],
            new("PK_orders", ["tenant_id", "order_id"]),
            []
        );
        var sql = new SqlServerSelectionSqlGenerator().Compile(
            new(new([orders], []), new(orders, "o"), new(orders.PrimaryKey), [], null)
        );

        Assert.StartsWith(
            "SELECT DISTINCT [o].[tenant_id] AS [__datapitcher_key_0], [o].[order_id] AS [__datapitcher_key_1]",
            sql.CommandText,
            StringComparison.Ordinal
        );
    }
    ...
    public void Compile_QuotesIdentifiersAndKeepsSqlMetacharactersInParameters()
    {
        ...
        var sql = new SqlServerSelectionSqlGenerator().Compile(
            PredicateQuery(new TextPredicate(new("o", "name"), TextMatch.Contains, new(typeof(string), attack)))
        );
    ...
    public void Compile_RendersEveryValidatedPredicateAsParameterizedSql(
    ...
    public void Compile_PairsCompositeForwardForeignKeyColumns()
    {
        ...
        Assert.Contains(
            ...
            new SqlServerSelectionSqlGenerator().Compile(query).CommandText,
            StringComparison.Ordinal
        );
    }
    ...
    {
        var sql = new SqlServerSelectionSqlGenerator().Compile(
            PredicateQuery(new ExistsPredicate(Lines, "l", [new(new("o", "id"), "order_id")], null, false))
        );

        Assert.Contains(
            "EXISTS (SELECT 1 FROM [dbo].[order_lines] AS [l] WHERE [o].[id] = [l].[order_id])",
            sql.CommandText,
            StringComparison.Ordinal
        );
    ...
    public void Compile_RendersEveryExistsCorrelation()
    {
        ...
        var lines = new TableDefinition(
            "dbo",
            "order_lines",
            [new("order_tenant_id", typeof(int), false), new("order_id", typeof(int), false)],
            new("PK_order_lines", ["order_tenant_id", "order_id"]),
            []
        );
        var query = new SelectionQuery(
            new([orders, lines], []),
        ...
        Assert.Contains(
            "[o].[tenant_id] = [l].[order_tenant_id] AND [o].[id] = [l].[order_id]",
            sql.CommandText,
            StringComparison.Ordinal
        );
    }
    ...
    public static IEnumerable<object?[]> PredicateCases()
    {
        ...
        [
            ...
            "NOT EXISTS (SELECT 1 FROM [dbo].[order_lines] AS [l] WHERE [o].[id] = [l].[order_id] AND [l].[id] = @p0)",
            new object[] { 7 },
        ];
    }

    private static readonly TableDefinition PredicateOrders = new(
        "dbo",
        "orders",
        [
            new("id", typeof(int), false),
    ...
    private static readonly TableDefinition Lines = new(
        "dbo",
        "order_lines",
        [new("id", typeof(int), false), new("order_id", typeof(int), false)],
        new("PK_order_lines", ["id"]),
        []
    );
    ...
    private static SelectionQuery OrdersJoinedToLines()
    {
        ...
        var lines = new TableDefinition(
            "dbo",
            "order_lines",
            [new("line_id", typeof(int), false), new("order_id", typeof(int), false)],
            new("PK_order_lines", ["line_id"]),
            []
        );
        return new(
            new([orders, lines], []),
    ...
    private static SelectionQuery PredicateQuery(SelectionPredicate predicate) =>
        new(
            new([PredicateOrders, Lines], []),
            new(PredicateOrders, "o"),
            new(PredicateOrders.PrimaryKey),
            [],
            predicate
        );
