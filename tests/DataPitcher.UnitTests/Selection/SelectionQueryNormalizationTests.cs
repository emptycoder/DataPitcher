...
public sealed class SelectionQueryNormalizationTests
{
    ...
    {
        var joined = SelectionQueryTestData.QuotedRootAndJoin(); var withoutJoin = new SelectionQuery(joined.Schema, joined.Root, joined.RootStableKey, [], null);
    ...
    {
        var query = SelectionQueryTestData.Query(new ComparisonPredicate(new("o", "Id"), comparison, new(typeof(int), 1)));
    ...
    {
        var isNull = SelectionQueryFingerprint.CanonicalText(SelectionQueryTestData.Query(new NullPredicate(new("o", "Name"), false)));
        var isNotNull = SelectionQueryFingerprint.CanonicalText(SelectionQueryTestData.Query(new NullPredicate(new("o", "Name"), true)));
        Assert.Contains("where(null(o.Name))", isNull); Assert.Contains("where(notnull(o.Name))", isNotNull); Assert.NotEqual(isNull, isNotNull);
    }
    [Theory]
    ...
    {
        var query = SelectionQueryTestData.Query(new TextPredicate(new("o", "Name"), match, new(typeof(string), "match")));
        Assert.Contains("where(" + name + "(o.Name:system.string:\"match\"))", SelectionQueryFingerprint.CanonicalText(query));
    }
    [Fact]
    ...
    public void Fingerprint_IdentifiesTemporalRangePredicate()
    {
        ...
        Assert.Contains("where(date(o.Day:system.dateonly:\"2026-09-02\":system.dateonly:\"2026-09-03\"))", SelectionQueryFingerprint.CanonicalText(query));
    }
    [Fact]
    public void Fingerprint_DistinguishesExistsNegation()
    {
        ...
        var exists = SelectionQueryFingerprint.CanonicalText(queries.Single(x => !((ExistsPredicate)x.Predicate!).Negated));
        var notExists = SelectionQueryFingerprint.CanonicalText(queries.Single(x => ((ExistsPredicate)x.Predicate!).Negated));
        Assert.Contains("where(exists(", exists); Assert.Contains("where(notexists(", notExists); Assert.NotEqual(exists, notExists);
    }
    [Fact]
    public void Normalize_CollapsesDoubleNegation()
    {
        var normalized = SelectionQueryNormalizer.Normalize(SelectionQueryTestData.Query(new NotPredicate(new NotPredicate(SelectionQueryTestData.Id(1)))));
        Assert.Equal(SelectionQueryTestData.Id(1), normalized.Predicate);
    }
    [Fact]
    public void Normalize_SortsAndDeduplicatesSetValues()
    {
        var query = SelectionQueryTestData.Query(new SetPredicate(new("o", "Id"), false, [new(typeof(int), 2), new(typeof(int), 1), new(typeof(int), 2)]));
        var normalized = Assert.IsType<SetPredicate>(SelectionQueryNormalizer.Normalize(query).Predicate);
        Assert.Equal([1, 2], normalized.Values.Select(x => (int)x.Value)); Assert.Contains("where(in(o.Id:system.int32:1,system.int32:2))", SelectionQueryFingerprint.CanonicalText(SelectionQueryTestData.Query(normalized)));
    }
    [Fact]
    ...
    {
        var query = SelectionQueryTestData.Query(new BetweenPredicate(new("o", "Id"), new(typeof(int), 1), new(typeof(int), 2)));
        Assert.Contains("where(between(o.Id:system.int32:1:system.int32:2))", SelectionQueryFingerprint.CanonicalText(query));
    }
    [Fact]
    public void Fingerprint_DistinguishesSetNegation()
    {
        var included = SelectionQueryFingerprint.CanonicalText(SelectionQueryTestData.Query(new SetPredicate(new("o", "Id"), false, [new(typeof(int), 1)])));
        var excluded = SelectionQueryFingerprint.CanonicalText(SelectionQueryTestData.Query(new SetPredicate(new("o", "Id"), true, [new(typeof(int), 1)])));
        Assert.Contains("where(in(o.Id:system.int32:1))", included); Assert.Contains("where(notin(o.Id:system.int32:1))", excluded); Assert.NotEqual(included, excluded);
    }
    [Fact]
    public void Normalize_FlattensNestedDisjunction()
    {
        var normalized = Assert.IsType<OrPredicate>(SelectionQueryNormalizer.Normalize(SelectionQueryTestData.Query(new OrPredicate([SelectionQueryTestData.Id(1), new OrPredicate([SelectionQueryTestData.Id(2), SelectionQueryTestData.Id(3)])]))).Predicate);
        Assert.Equal(3, normalized.Terms.Count);
    }
    [Fact]
    public void Fingerprint_RejectsUnknownPredicateWhenSelectionInvariantIsBypassed()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => SelectionQueryFingerprint.CanonicalText(SelectionQueryTestData.WithUnvalidatedPredicate(new UnknownSelectionPredicate())));
    }
    [Fact]
    public void Fingerprint_RejectsUndefinedComparisonOperatorWhenSelectionInvariantIsBypassed()
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => SelectionQueryFingerprint.CanonicalText(SelectionQueryTestData.WithUnvalidatedPredicate(new ComparisonPredicate(new("o", "Id"), (SelectionComparison)99, new(typeof(int), 1)))));
    }
}
