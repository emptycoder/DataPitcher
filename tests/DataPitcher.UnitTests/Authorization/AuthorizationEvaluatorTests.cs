...
public sealed class AuthorizationEvaluatorTests
{
    ...
    public void AuthorizationEvaluator_WhenPrincipalIsTerminal_ReturnsDeniedBeforeGrantUnion(
        PrincipalAuthorizationState state
    )
    {
        var decision = AuthorizationEvaluator.Evaluate(
            Input(state, [new(Role.Administrator, RoleGrantSource.ControlDatabaseAssignment)]),
            Permissions.AuthProvidersManage
        );
    ...
    public void AuthorizationEvaluator_WhenPositiveSourcesGrantDifferentRoles_UsesTheirUnion()
    {
        ...
        var decision = AuthorizationEvaluator.Evaluate(
            Input(PrincipalAuthorizationState.Active, grants),
            Permissions.AuthProvidersManage
        );
    ...
    {
        var groups =
            state == GroupResolutionState.Complete
                ? GroupResolutionResult.Complete([])
        ...
        Assert.Equal(
            ...
            AuthorizationEvaluator
                .Evaluate(
                    Input(PrincipalAuthorizationState.Active, [], groups, [Role.Operator]),
    ...
    public void AuthorizationEvaluator_WhenIndeterminateGroupsOnlyHaveUnrecognizedRoles_ReturnsDenied() =>
        Assert.Equal(
            ...
            AuthorizationEvaluator
                .Evaluate(
                    Input(PrincipalAuthorizationState.Active, [], GroupResolutionResult.Indeterminate(), [(Role)99]),
                    Permissions.TransfersStart
                )
                .Outcome
        );
    ...
    public void AuthorizationEvaluator_WhenKnownGrantExists_DoesNotTurnItIntoIndeterminate()
    {
        var decision = AuthorizationEvaluator.Evaluate(
            Input(
                PrincipalAuthorizationState.Active,
                [new(Role.Operator, RoleGrantSource.ControlDatabaseAssignment)],
                GroupResolutionResult.Indeterminate(),
                [Role.Planner]
            ),
            Permissions.TransfersStart
    ...
    {
        var decision = AuthorizationEvaluator.Evaluate(
            Input(PrincipalAuthorizationState.Active, [new((Role)99, RoleGrantSource.ControlDatabaseAssignment)]),
            Permissions.TransfersStart
        );
    ...
    public void AuthorizationEvaluator_WhenPresentationClaimsChange_UsesOnlyTheImmutableAuthorizationKey()
    {
        ...
        var first = Input(
            PrincipalAuthorizationState.Active,
            grants,
            principal: new(key, new("Ada", "ada@example.test", "ada", "ada@example.test"))
        );
        var renamed = Input(
            PrincipalAuthorizationState.Active,
            grants,
            principal: new(key, new("Grace", "grace@example.test", "grace", "grace@example.test"))
        );

        Assert.Equal(
            AuthorizationEvaluator.Evaluate(first, Permissions.TransfersStart),
            AuthorizationEvaluator.Evaluate(renamed, Permissions.TransfersStart)
        );
    }
    ...
    {
        var disabled = Input(
            PrincipalAuthorizationState.Disabled,
            [new(Role.Operator, RoleGrantSource.ControlDatabaseAssignment)]
        );
        var enabled = Input(PrincipalAuthorizationState.Active, disabled.PositiveGrants);
        var change = new PrincipalStateChange(
            disabled.Principal.AuthorizationKey,
            PrincipalAuthorizationState.Disabled,
            PrincipalAuthorizationState.Active,
            disabled.Principal.AuthorizationKey,
        ...
        Assert.Equal(
            AuthorizationOutcome.Denied,
            AuthorizationEvaluator.Evaluate(disabled, Permissions.TransfersStart).Outcome
        );
        Assert.True(change.RequiresAudit);
        Assert.Equal(disabled.PositiveGrants, enabled.PositiveGrants);
        Assert.Equal(
            AuthorizationOutcome.Granted,
            AuthorizationEvaluator.Evaluate(enabled, Permissions.TransfersStart).Outcome
        );
    }
    ...
    public void PrincipalStateChange_WhenTerminalDenyIsLifted_RequiresAudit()
    {
        ...
        var change = new PrincipalStateChange(
            key,
            PrincipalAuthorizationState.TerminalDeny,
            PrincipalAuthorizationState.Active,
            key,
            DateTimeOffset.UnixEpoch
        );
    ...
    private static AuthorizationInput Input(
        PrincipalAuthorizationState state,
        IEnumerable<RoleGrant> grants,
        GroupResolutionResult? groups = null,
        IEnumerable<Role>? possibleRoles = null,
        NormalizedPrincipal? principal = null
    ) =>
        new(
            principal
                ?? new(
                    new("provider", "https://issuer.example", "tenant", PrincipalKind.User, "subject"),
                    new(null, null, null, null)
                ),
            state,
            grants,
            groups ?? GroupResolutionResult.NotApplicable(),
            possibleRoles ?? []
        );
}
