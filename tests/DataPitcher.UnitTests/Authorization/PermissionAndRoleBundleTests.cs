...
public sealed class PermissionAndRoleBundleTests
{
    ...
    public void Permissions_ExposeTheCompleteClosedVocabulary() =>
        Assert.Equal(
            ...
            Permissions.All.Select(permission => permission.Value)
        );
    ...
    public void RoleBundles_ViewerHasEveryReadPermissionOnly() =>
        Assert.Equal(
            new PermissionSet([
                ...
                Permissions.SelectionsRead,
                Permissions.TransfersRead,
            ]),
            RoleBundles.For(Role.Viewer)
        );
    ...
    public void RoleBundles_PlannerAddsPlanningWritesAndNoTransferStart() =>
        Assert.Equal(
            new PermissionSet([
                ...
                Permissions.SelectionsRawSql,
                Permissions.SelectionsRead,
                Permissions.SelectionsWrite,
                Permissions.TransfersRead,
            ]),
    ...
    public void RoleBundles_OperatorAddsTransferExecutionAndOverrides() =>
        Assert.Equal(
            new PermissionSet([
                ...
                Permissions.SchemaRead,
                Permissions.SelectionsRead,
                Permissions.TransfersConstraintOverride,
                Permissions.TransfersRead,
                Permissions.TransfersStart,
                Permissions.TransfersTriggerOverride,
                Permissions.TransfersUsePotentiallyLossyMapping,
            ]),
            RoleBundles.For(Role.Operator)
        );

    [Fact]
    public void RoleBundles_AdministratorHasEveryPermission() =>
        Assert.Equal(new PermissionSet(Permissions.All), RoleBundles.For(Role.Administrator));
    ...
    public void PermissionSet_UnionAndRemovalRemainImmutableAndMonotonic()
    {
        ...
        Assert.Equal(
            full.GetHashCode(),
            new PermissionSet([Permissions.PlansSeal, Permissions.TransfersStart]).GetHashCode()
        );
