...
public sealed class ExternalPrincipalKeyTests
{
    ...
    public void ExternalPrincipalKey_WhenAnyImmutableComponentDiffers_IsNotEqual()
    {
        ...
        Assert.NotEqual(
            key,
            new ExternalPrincipalKey("entra-prod", "https://login.example/tenant", null, PrincipalKind.User, "object-a")
        );
        Assert.NotEqual(
            key,
            new ExternalPrincipalKey(
    ...
    public void ExternalPrincipalKey_WhenRequiredIdentityPartIsInvalid_Throws(
        string provider,
        string issuer,
        string subject
    ) =>
        Assert.Throws<ArgumentException>(() =>
            new ExternalPrincipalKey(provider, issuer, "tenant-a", PrincipalKind.User, subject)
        );

    [Fact]
    public void ExternalPrincipalKey_WhenTenantIsEmptyRatherThanNull_Throws() =>
        Assert.Throws<ArgumentException>(() =>
            new ExternalPrincipalKey("provider", "https://login.example/tenant", "", PrincipalKind.User, "subject")
        );
    ...
    public void NormalizedPrincipal_WhenPresentationClaimsChange_KeepsTheSameAuthorizationKey()
    {
        ...
        var first = new NormalizedPrincipal(
            key,
            new PrincipalPresentation("Ada", "ada@example.test", "ada", "ada@example.test")
        );
        var renamed = new NormalizedPrincipal(
            key,
            new PrincipalPresentation("Grace", "grace@example.test", "grace", "grace@example.test")
        );
