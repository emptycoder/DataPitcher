...
public sealed class JobStateMachineTests
{
    ...
    [
        ...
        (JobState.Pausing, JobState.Failed),
        (JobState.Pausing, JobState.Queued),
        ...
        (JobState.Verifying, JobState.VerificationFailed),
    ];

    public static IEnumerable<object[]> StatePairs() =>
        Enum.GetValues<JobState>()
            .SelectMany(from => Enum.GetValues<JobState>().Select(to => new object[] { from, to }));
    ...
    {
        Assert.Equal(
            Allowed.Where(pair => pair.From == from).Select(pair => pair.To),
            JobStateMachine.ValidTargets(from)
        );
