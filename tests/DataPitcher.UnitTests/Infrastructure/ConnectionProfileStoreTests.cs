...
public sealed class ConnectionProfileStoreTests
{
    ...
    public async Task ConnectionProfileStore_WhenRead_ReturnsNoSecretLocatorOrContent()
    {
        ...
        try
        {
            ...
            var stored = db.Query<string>(
                    "SELECT ConnectionId || '|' || DisplayName || '|' || ProviderId || '|' || SecretReferenceKind || '|' || SecretReferenceLocator || '|' || BusinessSchema || '|' || StagingSchema || '|' || HealthState || '|' || IdempotencyKey FROM ConnectionProfiles"
                )
                .Single();
    ...
    public async Task ConnectionProfileStore_WhenUpdatedWithCurrentEtag_ReplacesTheReferenceAndVersion()
    {
        ...
        var updated = await store.UpdateAsync(
            ...
            new ConnectionProfileDraft(
                "Target",
                "sqlserver",
                new(SecretReferenceKind.EnvironmentVariable, "DP_UPDATED_SECRET"),
                "dbo",
                "__datapitcher"
            ),
            "\"1\"",
    ...
    public async Task ConnectionProfileStore_WhenUpdatedWithOldEtag_FailsWithoutChangingTheProfile()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.UpdateAsync(created.ConnectionId, Draft("stale"), "\"1\"", CancellationToken.None)
        );
    ...
    public async Task ConnectionProfileStore_WhenDeletedWithOldEtag_FailsWithoutDeletingTheProfile()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.DeleteAsync(created.ConnectionId, "\"1\"", CancellationToken.None)
        );
        var profile = await store.GetProfileAsync(created.ConnectionId, CancellationToken.None);
        ...
        using (var db = fixture.Database.Open())
        {
            db.Execute(
                ...
                new DataParameter[]
                {
                    ...
                    new("idempotencyKey", "scan-01"),
                    new("state", "Queued"),
                    new("createdUtc", fixture.Clock.UtcNow.ToString("O")),
                    new("updatedUtc", fixture.Clock.UtcNow.ToString("O")),
                }
            );
            db.Execute(
    ...
    public async Task ConnectionProfileStore_WhenAssessmentIsSaved_PersistsOnlySafeAssessmentData()
    {
        ...
        var assessment = new ConnectionAssessment(
            ...
            new[] { ConnectionCapability.SupportsDurableResume },
            "staging_cleanup_failed"
        );

        var summary = await store.SaveAssessmentAsync(
            created.ConnectionId,
        ...
        Assert.Equal(
            "staging_cleanup_failed",
            db.Query<string>("SELECT CleanupFailureCode FROM ConnectionProfiles").Single()
        );
        Assert.DoesNotContain(
            "DP_TEST_SECRET",
            db.Query<string>("SELECT CapabilitiesJson FROM ConnectionProfiles").Single(),
    ...
    public async Task ConnectionProfileStore_WhenAssessmentHasNoCleanupFailure_PersistsNoFailureCode()
    {
        ...
        _ = await store.SaveAssessmentAsync(
            created.ConnectionId,
    ...
    public async Task ConnectionProfileStore_WhenPersistingProfileOrAssessment_EmitsNoReferenceOrSecretInLogsOrActivity()
    {
        var forbidden = new[]
        {
            ...
            "Host=db;Password=connection-string-sentinel",
        ...
        using var db = fixture.Database.Open();
        var stored = db.Query<string>(
                "SELECT COALESCE(CleanupFailureCode, '') || '|' || COALESCE(CapabilitiesJson, '') FROM ConnectionProfiles"
            )
            .Single();
    ...
    public async Task ConnectionProfileStore_WhenIdempotencyKeyIsBlankOrProfileIsMissing_FailsWithFixedSafeErrors()
    {
        ...
        var missing = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.GetSummaryAsync(Guid.NewGuid(), CancellationToken.None)
        );
        ...
        var malformed = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.UpdateAsync(created.ConnectionId, Draft(), "invalid", CancellationToken.None)
        );
        var zero = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            store.DeleteAsync(created.ConnectionId, "\"0\"", CancellationToken.None)
        );
    ...
    private static ConnectionProfileDraft Draft(string displayName = "Source") =>
        new(
            displayName,
            "postgresql",
            new(SecretReferenceKind.EnvironmentVariable, "DP_TEST_SECRET"),
            "app",
            "__datapitcher"
        );
