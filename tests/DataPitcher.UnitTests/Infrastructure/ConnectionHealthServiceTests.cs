...
public sealed class ConnectionHealthServiceTests
{
    ...
    public async Task RecheckAsync_MarksCheckingBeforeResolvingAndPersistsUnhealthyForMissingRequirements()
    {
        ...
        var resolver = new Resolver { BeforeResolve = async () => (await store.GetSummaryAsync(profile.ConnectionId, CancellationToken.None)).Health };
        var service = new ConnectionHealthService(store, resolver, new ConnectionProviderRegistry(new IConnectionProvider[] { new Provider(new Detector(new ConnectionProbeEvidence("identity", "version", new[] { ConnectionCapability.CanConnect }, null))) }));

        var summary = await service.RecheckAsync(profile.ConnectionId, TransferMode.DirectFast, ConnectionRole.Source, CancellationToken.None);
    ...
    public async Task RevalidateAsync_WhenTargetIsNotHealthy_ProbesBothLoadedConnectionRolesAndThrowsSafely()
    {
        ...
        var detector = new Detector(Evidence(), new ConnectionProbeEvidence("identity", "version", new[] { ConnectionCapability.CanConnect }, null));
        var service = new ConnectionHealthService(store, new Resolver(), new ConnectionProviderRegistry(new IConnectionProvider[] { new Provider(detector) }));
        ...
        var exception = await Assert.ThrowsAsync<ConnectionNotHealthyException>(() => service.RevalidateAsync(run, CancellationToken.None));
    ...
    public async Task RevalidateAsync_WhenSourceIsNotHealthy_DoesNotProbeTheTarget()
    {
        ...
        var detector = new Detector(new ConnectionProbeEvidence("identity", "version", Array.Empty<ConnectionCapability>(), null));
        var service = new ConnectionHealthService(store, new Resolver(), new ConnectionProviderRegistry(new IConnectionProvider[] { new Provider(detector) }));
        var run = new TransferRun(Guid.NewGuid(), Guid.NewGuid(), "seal", true, source.ConnectionId, target.ConnectionId, TransferMode.DirectFast);

        await Assert.ThrowsAsync<ConnectionNotHealthyException>(() => service.RevalidateAsync(run, CancellationToken.None));
    ...
    private sealed class Resolver : ISecretReferenceResolver
    {
        ...
        {
            if (BeforeResolve is not null) HealthAtResolve = await BeforeResolve();
