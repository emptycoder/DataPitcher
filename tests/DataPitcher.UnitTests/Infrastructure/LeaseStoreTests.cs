...
public sealed class LeaseStoreTests
{
    ...
    public void LeaseStore_WhenExpiredOwnerIsReplaced_IncrementsTheMonotonicFence()
    {
        ...
        var store = new LeaseStore(fixture.Database, fixture.Clock);
        ...
        var second = store.Acquire(jobId, "worker-b", ttl);
    ...
    public void LeaseStore_WhenRenewalIsDue_RenewsAtTwoThirdsOfTtlWithoutSleeping()
    {
        ...
        fixture.Clock.Advance(TimeSpan.FromSeconds(39));
        Assert.Equal(lease, store.RenewIfDue(lease, ttl));
        fixture.Clock.Advance(TimeSpan.FromSeconds(1));
        var renewed = store.RenewIfDue(lease, ttl);
        Assert.NotNull(renewed);
        Assert.Equal(lease.FenceToken, renewed!.FenceToken);
        Assert.True(renewed.ExpiresUtc > lease.ExpiresUtc);
    }
