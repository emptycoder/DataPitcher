...
public sealed class JobStoreTests
{
    ...
    public void JobStore_WhenStartIsDuplicated_ReturnsTheOriginalQueuedJob()
    {
        ...
        var first = store.Start(new(Guid.NewGuid(), "start-42"));
        var duplicate = store.Start(new(Guid.NewGuid(), "start-42"));
        Assert.True(first.Created);
        Assert.False(duplicate.Created);
        Assert.Equal(first.Job.JobId, duplicate.Job.JobId);
        Assert.Equal(JobState.Queued, first.Job.State);
    }
    ...
    public void JobStore_WhenStarted_ReadsBackEveryPersistedField()
    {
        ...
        Assert.Equal(job.RunId, read.RunId);
        Assert.Equal(job.PlanId, read.PlanId);
        Assert.Equal(job.IdempotencyKey, read.IdempotencyKey);
        Assert.Equal(job.State, read.State);
    }
    ...
    public void JobStore_WhenLeaseIsCurrent_PersistsTheStateChangeAndItsHistory()
    {
        ...
        fixture.Migrator.Apply();
        var store = new JobStore(fixture.Database, fixture.Clock);
        var job = store.Start(new(Guid.NewGuid(), "start-43")).Job;
        var lease = new LeaseStore(fixture.Database, fixture.Clock).Acquire(
            job.JobId,
            "worker-a",
        ...
        Assert.Equal(JobState.Preparing, result.Job!.State);
        Assert.Equal(
            [(JobState.Draft, JobState.Queued), (JobState.Queued, JobState.Preparing)],
    ...
    public async Task JobStore_WhenFirstOwnerIsStale_ItsGuardedWriteAffectsZeroRows()
    {
        ...
        fixture.Migrator.Apply();
        var store = new JobStore(fixture.Database, fixture.Clock);
        var job = store.Start(new(Guid.NewGuid(), "start-44")).Job;
        ...
        var takeoverComplete = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
        var staleWrite = Task.Run(async () =>
        {
            firstReady.SetResult(true);
            await takeoverComplete.Task;
            return store.TryTransition(first, JobState.Preparing);
        });
        await firstReady.Task;
        fixture.Clock.Advance(ttl.Add(TimeSpan.FromTicks(1)));
        var second = leases.Acquire(job.JobId, "worker-b", ttl)!;
        takeoverComplete.SetResult(true);
        var stale = await staleWrite;
        Assert.True(second.FenceToken > first.FenceToken);
        Assert.Equal(0, stale.RowsAffected);
        Assert.DoesNotContain(store.GetHistory(job.JobId), x => x == (JobState.Queued, JobState.Preparing));
    }
    ...
    public void JobStore_WhenOwnerReacquiresAfterExpiry_RejectsTheStaleFenceEvenWithMatchingOwner()
    {
        ...
        fixture.Migrator.Apply();
        var store = new JobStore(fixture.Database, fixture.Clock);
        var job = store.Start(new(Guid.NewGuid(), "start-46")).Job;
        var leases = new LeaseStore(fixture.Database, fixture.Clock);
        var ttl = TimeSpan.FromMinutes(1);
        var first = leases.Acquire(job.JobId, "worker-a", ttl)!;
        fixture.Clock.Advance(ttl.Add(TimeSpan.FromTicks(1)));
        ...
        Assert.Equal(first.OwnerId, reacquired.OwnerId);
        Assert.True(reacquired.FenceToken > first.FenceToken);
        Assert.Equal(0, stale.RowsAffected);
        Assert.DoesNotContain(store.GetHistory(job.JobId), x => x == (JobState.Queued, JobState.Preparing));
    }
}
