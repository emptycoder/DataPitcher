...
public sealed class ControlDatabaseMigratorTests
{
    ...
    public void ControlDatabaseMigrator_WhenAppliedTwice_RecordsEveryRegisteredMigrationExactlyOnce()
    {
        ...
        Assert.Equal(["ConnectionId", "DisplayName", "ProviderId", "SecretReferenceKind", "SecretReferenceLocator", "BusinessSchema", "StagingSchema", "Version", "HealthState", "AssessmentMode", "AssessmentRole", "DatabaseIdentity", "ProviderVersion", "CapabilitiesJson", "CleanupFailureCode", "CreatedUtc", "UpdatedUtc", "IdempotencyKey"], db.Query<string>("SELECT name FROM pragma_table_info('ConnectionProfiles') ORDER BY cid").ToList());
        Assert.Equal(["ScanId", "ConnectionId", "IdempotencyKey", "State", "SnapshotId", "SnapshotHash", "FailureCode", "CreatedUtc", "UpdatedUtc"], db.Query<string>("SELECT name FROM pragma_table_info('SchemaScans') ORDER BY cid").ToList());
        Assert.Equal(["SnapshotId", "ConnectionId", "SnapshotHash", "ContentJson", "CreatedUtc"], db.Query<string>("SELECT name FROM pragma_table_info('SchemaSnapshots') ORDER BY cid").ToList());
    }
}
