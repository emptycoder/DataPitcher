...
public sealed class JobEventSignalTests
{
    ...
    public async Task JobEventSignal_WhenAnEventCommitsBeforeWaiterRegistration_DoesNotMissTheNotification()
    {
        ...
        var signal = new JobEventSignal(() => { beforeRegistration.SignalAndWait(); afterPublication.SignalAndWait(); });
        ...
        var appended = await events.AppendAsync(new JobEventAppend(jobId, "progress", new JobEventPayload("running", 10, 100)), CancellationToken.None);
