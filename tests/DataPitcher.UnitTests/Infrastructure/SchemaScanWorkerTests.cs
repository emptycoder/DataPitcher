...
public sealed class SchemaScanWorkerTests
{
    ...
    public async Task ProcessNextAsync_QueuesProcessesAndProjectsAnImmutableSnapshot()
    {
        ...
        var worker = new SchemaScanWorker(
            ...
            profiles,
            new Resolver(),
            new ConnectionProviderRegistry(new IConnectionProvider[] { new Provider(introspector) })
        );
        ...
        var neighbourhood = await snapshots.GetNeighbourhoodAsync(
            profile.ConnectionId,
            snapshot.SnapshotId,
            "app",
            "Orders",
        ...
        Assert.Single(
            graph.Edges,
            edge =>
                edge.Child == new SchemaTableAddress("app", "Orders")
                && edge.Parent == new SchemaTableAddress("app", "Customers")
        );
    ...
    public async Task ProcessNextAsync_WhenScanIsReplayedOrMetadataChanges_ReusesOrCreatesTheExpectedSnapshot()
    {
        ...
        introspector.Content = new SchemaSnapshotContent(
            new[] { Orders("OrderNumber"), Customers() },
            new[] { ForeignKey() }
        );
    ...
    public async Task ProcessNextAsync_WhenResolutionOrIntrospectionFails_StoresOnlyFixedFailureCodes()
    {
        ...
        var worker = new SchemaScanWorker(
            ...
            profiles,
            new Resolver(),
            new ConnectionProviderRegistry(new IConnectionProvider[] { new Provider(new ThrowingIntrospector()) })
        );
    ...
    public async Task ProcessNextAsync_WhenTheProviderIsNotRegistered_StoresTheUnsupportedProviderFailure()
    {
        ...
        var profile = await profiles.CreateAsync(
            new ConnectionProfileDraft(
                "Source",
                "unregistered",
                new SecretReference(SecretReferenceKind.EnvironmentVariable, "DP_SCAN_SECRET"),
                "app",
                "__datapitcher"
            ),
            "profile-unsupported-provider",
            CancellationToken.None
        );
        var snapshots = new SchemaSnapshotStore(fixture.Database, fixture.Clock);
        var scan = await snapshots.QueueAsync(
            profile.ConnectionId,
            "scan-unsupported-provider",
            CancellationToken.None
        );
        var worker = new SchemaScanWorker(
            snapshots,
            profiles,
            new Resolver(),
            new ConnectionProviderRegistry(Array.Empty<IConnectionProvider>())
        );
    ...
    public async Task ExecuteAsync_WhenStoppedBeforePolling_LeavesQueuedScansUntouched()
    {
        ...
        Assert.Equal(
            SchemaScanState.Queued,
            (await snapshots.GetScanAsync(profile.ConnectionId, queued.ScanId, CancellationToken.None)).State
        );
    }
    ...
    public async Task ExecuteAsync_WhenRunning_PollsAndCompletesQueuedScansUntilStopped()
    {
        ...
        var worker = new SchemaScanWorker(
            ...
            new ConnectionProviderRegistry(
                new IConnectionProvider[] { new Provider(new ImmediateIntrospector(Content())) }
            )
        );
    ...
    public async Task GetScanAsync_WhenTheScanDoesNotBelongToTheConnection_FailsWithTheFixedNotFoundError()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            snapshots.GetScanAsync(Guid.NewGuid(), Guid.NewGuid(), CancellationToken.None)
        );
    ...
    public async Task GetAsync_WhenTheSnapshotDoesNotBelongToTheConnection_FailsWithTheFixedNotFoundError()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            snapshots.GetAsync(profile.ConnectionId, Guid.NewGuid(), CancellationToken.None)
        );
        ...
        using (var db = fixture.Database.Open())
            db.Execute(
                ...
                new ControlParameter[]
                {
                    ...
                    new("snapshotHash", "invalid"),
                    new("contentJson", "null"),
                    new("createdUtc", fixture.Clock.UtcNow.ToString("O")),
                }
            );
        var snapshots = new SchemaSnapshotStore(fixture.Database, fixture.Clock);

        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            snapshots.GetAsync(profile.ConnectionId, snapshotId, CancellationToken.None)
        );
    ...
    public async Task GetNeighbourhoodAsync_WhenDepthIsZero_RejectsTheInvalidProjectionRequest()
    {
        ...
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            snapshots.GetNeighbourhoodAsync(
                profile.ConnectionId,
                snapshot.SnapshotId,
                "app",
                "Orders",
    ...
    public async Task ClaimNextAsync_WhenTheClaimIsLost_ReturnsNoClaimAndLeavesTheScanQueued()
    {
        ...
        using (var db = fixture.Database.Open())
            db.Execute(
                "CREATE TRIGGER lose_schema_claim BEFORE UPDATE OF State ON SchemaScans WHEN NEW.State = 'Running' BEGIN SELECT RAISE(IGNORE); END;"
            );
    ...
    public async Task GetTableAsync_WhenTheTableHasNoPrimaryKey_PreservesTheAbsentKey()
    {
        ...
        var snapshots = new SchemaSnapshotStore(fixture.Database, fixture.Clock);
        var content = new SchemaSnapshotContent(
            new[]
            {
                new SchemaTable(
                    "app",
                    "Audit",
                    new[] { new SchemaColumn("Message", "text", "System.String", true) },
                    null,
                    Array.Empty<SchemaKey>()
                ),
            },
            Array.Empty<SchemaForeignKey>()
        );
        var snapshot = await StoreSnapshotAsync(snapshots, profile, content);
    ...
    private static ConnectionProfileDraft Draft() =>
        new(
            "Source",
            "postgresql",
            new SecretReference(SecretReferenceKind.EnvironmentVariable, "DP_SCAN_SECRET"),
    ...
    private static async Task<StoredSchemaSnapshot> StoreSnapshotAsync(
    ...
    )
    {
        ...
        var scan =
            await snapshots.ClaimNextAsync(CancellationToken.None)
            ?? throw new InvalidOperationException("Schema scan was not claimed.");
    ...
    private static Task ExecuteAsync(SchemaScanWorker worker, CancellationToken cancellationToken) =>
        (Task)(
            typeof(SchemaScanWorker)
                .GetMethod(
                    "ExecuteAsync",
                    System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic
                )!
                .Invoke(worker, [cancellationToken])
            ?? throw new InvalidOperationException("Schema scan worker did not start.")
        );
    ...
    private static async Task WaitForStateAsync(
        ...
        Guid connectionId,
        Guid scanId,
        SchemaScanState expected
    )
    {
        for (var attempt = 0; attempt < 100; attempt++)
        {
            if ((await snapshots.GetScanAsync(connectionId, scanId, CancellationToken.None)).State == expected)
                return;
            await Task.Delay(10);
        }
    ...
    private static SchemaTable Orders(string column = "CustomerId") =>
        new(
            "app",
            "Orders",
            new[]
            {
                new SchemaColumn("Id", "int", "System.Int32", false),
                new SchemaColumn(column, "int", "System.Int32", false),
            },
            new SchemaKey("PK_Orders", new[] { "Id" }),
            Array.Empty<SchemaKey>()
        );

    private static SchemaTable Customers() =>
        new(
            "app",
            "Customers",
            new[] { new SchemaColumn("Id", "int", "System.Int32", false) },
            new SchemaKey("PK_Customers", new[] { "Id" }),
            Array.Empty<SchemaKey>()
        );
    ...
    private static SchemaForeignKey ForeignKey() =>
        new(
            "FK_Orders_Customers",
            new SchemaTableAddress("app", "Orders"),
            new SchemaTableAddress("app", "Customers"),
            new[] { "CustomerId" },
            new[] { "Id" },
            true,
            true
    ...
    private sealed class Resolver : ISecretReferenceResolver
    {
        ...
        {
            Calls++;
            return Task.FromResult("resolved-secret");
        }
    }
    ...
    {
        public Task<string> ResolveAsync(SecretReference reference, CancellationToken cancellationToken) =>
            Task.FromException<string>(new InvalidOperationException("secret unavailable"));
    }
    ...
    private sealed class BlockingIntrospector(SchemaSnapshotContent content) : ISchemaIntrospector
    {
        ...
        private readonly TaskCompletionSource<bool> _startedAgain = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
        private readonly TaskCompletionSource<bool> _releasedAgain = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
        public int Calls { get; private set; }
        ...
        )
        {
            ...
            {
                _startedAgain.TrySetResult(true);
                await _releasedAgain.Task.WaitAsync(cancellationToken);
            }
            return Content;
        }
    }
