...
public sealed class HostSmokeTests(ApiWebApplicationFactory factory) : IClassFixture<ApiWebApplicationFactory>
{
    ...
    private static readonly IReadOnlySet<string> AllowedResourceIdentifierNames =
        new HashSet<string> { "connectionId", "planId", "jobId", "snapshotId", "selectionId", "operationId" };
    ...
    public void ResponseRecords_SerializeOnlyKnownResourceIdentifierNamesAsGuids(Type responseType)
    {
        ...
        var json = JsonSerializer.Serialize(instance, responseType, new JsonSerializerOptions(JsonSerializerDefaults.Web));
    ...
    private static object CreateSample(Type type)
    {
        ...
        var arguments = constructor.GetParameters().Select(parameter => CreateArgument(parameter.ParameterType, parameter.Name!, guid)).ToArray();
