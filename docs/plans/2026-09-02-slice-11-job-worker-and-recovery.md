...
Boundary-only cancellation preserves the target checkpoint invariant: a batch commits atomically with its checkpoint, or it does not commit. Cancelling an apply mid-transaction would tear apart exactly the state the checkpoint design prevents. Cancelling the linked run token would also prevent the required cleanup and `MarkCancelledAsync`; those operations instead use a separate cleanup token that remains uncancelled.
