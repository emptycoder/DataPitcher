...
| DP-PLAN-002 | A plan's column mapping is prefilled by name, editable per column and table, stored as overrides, and every target problem is raised before sealing and refused at sealing. | ADR 0008 §8; `dependency-semantics.md` §3 | Column mapping feature | `PlanMappingResolverTests`; `PlanSealingServiceTests` `SealAsync_WritesTheOperatorsColumnMappingIntoThePlan`, `…RefusesNamingIt`; `PlanStoreTests` overrides round trip; `EndpointSurfaceTests` mapping endpoint and save; provider end-to-end `Transfer_WhenThePlanMapsARenamedColumnAndExcludesAnother_WritesWhatTheMappingSays`, `Seal_WhenAMappedUniqueKeyColumnCollidesUnderItsTargetName_RefusesNamingBothRows`; `PlanSealingServiceTests` `SealAsync_WhenTheNullableCycleColumnIsRenamedOnTheTarget_StillDefersIt`; web `MappingPanel.test.ts`, `MappingPanel.render.test.tsx` | Planned |
| DP-TRANSFER-004 | Rows carry the source column names they were projected with; the write side resolves every value by name, never by a position rebuilt from the plan, so computed, generated and rowversion source columns cannot shift values into neighbouring target columns. | `architecture.md` §4; ADR 0008 §8 | Positional drift fix | `WorkerContractsTests` `TransferUnit_Value_*`; provider end-to-end `Transfer_WhenTheSourceHasAComputedColumnBeforeOtherColumns…`, `…RowVersionColumn…` / `…GeneratedColumnRightAfterTheKey…`, `…StableKeyIsSpelledInAnotherCase…` | Planned |
| DP-TRANSFER-005 | Stable keys of every integer, exact decimal, character, binary, unique identifier and date-time type round-trip through the checkpoint encoding to their exact CLR type; Int, BigInt and NVarChar keep their original bytes; a key type the provider cannot record is refused at seal time, never mid-write. | ADR 0001 (checkpoint holds the last stable key); `dependency-semantics.md` §3 | Stable-key type coverage | `StableKeyCodecTests` (per-type round trip, pinned bytes, mixed composite, approximate types refused); `PlanSealingServiceTests` `SealAsync_WhenAStableKeyTypeCannotBeRecorded_RefusesBeforeTheClosureNamingTheColumn`; provider end-to-end `Transfer_WhenTheStableKeyIsAVarcharColumn_WritesTheRows`, `Seal_WhenTheStableKeyTypeCannotBeRecorded_RefusesNamingTheColumnBeforeAnyRowIsWritten` | Planned |
