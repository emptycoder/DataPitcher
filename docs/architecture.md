...
3. **Dependencies and Plan.** The column mapping is prefilled by name from the source and target schemas and the operator adjusts it where the target differs; problems are shown before sealing. The closure runs, the target is probed, and DataPitcher validates the target schema and mapping, detects conflicts and cycles, and computes exact counts. The stage exits only when an immutable plan is sealed.
