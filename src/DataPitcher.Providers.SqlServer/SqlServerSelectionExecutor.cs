...
public sealed class SqlServerSelectionExecutor(string sourceConnectionString, SqlServerSchemaSnapshot schema) : ISelectionExecutor
{
    ...
    {
        if (selection.IsRawSql) RawSqlSafetyValidator.Validate(RawSqlDialect.SqlServer, selection.CommandText);
        await using var connection = await OpenAsync(cancellationToken);
        var source = Source(selection);
        await using var command = Command(connection, "/* DataPitcher.Selection.Validate */ " + source.Prefix + " SELECT TOP (1) * FROM " + source.From, selection);
    ...
    public async Task<SelectionKeySet> ReadKeysAsync(GeneratedSelectionSql selection, int maximumResultSize, CancellationToken cancellationToken)
    {
        ...
        await using var connection = await OpenAsync(cancellationToken);
        var source = Source(selection);
        await using var command = Command(connection, "/* DataPitcher.Selection.Keys */ " + source.Prefix + " SELECT DISTINCT TOP (@take) " + aliases + " FROM " + source.From + " ORDER BY " + aliases, selection);
    ...
    public async Task<SelectionPreview> PreviewAsync(GeneratedSelectionSql selection, int rowLimit, int textLimit, int binaryLimit, CancellationToken cancellationToken)
    {
        ...
        var projection = string.Join(", ", keys.Select(SqlServerIdentifier.Quote).Concat(root.Columns.Select(column => PreviewProjection(rootAlias, column))));
        var join = string.Join(" AND ", selection.RootStableKey.Columns.Select((column, index) => rootAlias + "." + SqlServerIdentifier.Quote(column) + " = keys." + SqlServerIdentifier.Quote(keys[index])));
        var order = string.Join(", ", selection.RootStableKey.Columns.Select(column => rootAlias + "." + SqlServerIdentifier.Quote(column)));
        var source = Source(selection);
        var cte = source.IsCte ? source.Prefix : "WITH selection AS (" + CommandText(selection) + "\n)";
        var sql = "/* DataPitcher.Selection.Preview */ " + cte + ", keys AS (SELECT DISTINCT TOP (@previewLimit) " + aliases + " FROM " + (source.IsCte ? source.From : "selection") + " ORDER BY " + aliases + ") SELECT " + projection + " FROM " + SqlServerIdentifier.Qualified(root.Schema, root.Name) + " AS " + rootAlias + " INNER JOIN keys ON " + join + " ORDER BY " + order;
        await using var connection = await OpenAsync(cancellationToken);
    ...
    public async Task<long> CountAsync(GeneratedSelectionSql selection, CancellationToken cancellationToken)
    {
        ...
        await using var connection = await OpenAsync(cancellationToken);
        var source = Source(selection);
        await using var command = Command(connection, "/* DataPitcher.Selection.Count */ " + source.Prefix + " SELECT COUNT_BIG(*) FROM (SELECT DISTINCT " + aliases + " FROM " + source.From + ") AS keys", selection);
    ...
    private static async Task<List<StableKey>> ReadKeysAsync(SqlCommand command, UniqueConstraint stableKey, CancellationToken cancellationToken)
    {
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
    ...
    private async Task<SelectionPreview> ReadPreviewAsync(SqlDataReader reader, TableDefinition root, UniqueConstraint stableKey, CancellationToken cancellationToken)
    {
    ...
    private static string Aliases(UniqueConstraint stableKey) => string.Join(", ", SelectionKeyAliases.ForKey(stableKey).Select(SqlServerIdentifier.Quote));
    ...
    private static string PreviewProjection(string rootAlias, ColumnDefinition column)
    {
        ...
        return column.ClrType == typeof(string)
            ? "CASE WHEN LEN(" + value + ") > @textLimit THEN LEFT(" + value + ", @textLimit) ELSE " + value + " END, CAST(CASE WHEN LEN(" + value + ") > @textLimit THEN 1 ELSE 0 END AS bit)"
            : column.ClrType == typeof(byte[])
                ? "CASE WHEN DATALENGTH(" + value + ") > @binaryLimit THEN SUBSTRING(" + value + ", 1, @binaryLimit) ELSE " + value + " END, CAST(CASE WHEN DATALENGTH(" + value + ") > @binaryLimit THEN 1 ELSE 0 END AS bit)"
                : value + ", CAST(0 AS bit)";
    }
