...
public sealed class SqlServerConnectionProbe : ICapabilityDetector
{
    ...
    private static async Task<ConnectionProbeEvidence> ReadEvidenceAsync(
        SqlConnection connection, ConnectionProbeRequest request, CancellationToken cancellationToken)
    {
    ...
    {
        await using var command = new SqlCommand("SELECT DB_NAME(), CONVERT(nvarchar(128), SERVERPROPERTY('ProductVersion'));", connection)
        {
            CommandTimeout = 5,
        };
    ...
    private static async Task<(bool CanReadSchema, bool CanReadBusinessRows, bool CanCreateStaging, bool CanWriteBusinessRows, bool CanPreserveIdentity, bool CanUseSnapshotIsolation)> ReadPermissionsAsync(
        SqlConnection connection, ConnectionProfile profile, CancellationToken cancellationToken)
    {
        const string sql = "SELECT HAS_PERMS_BY_NAME(DB_NAME(), 'DATABASE', 'CONNECT'), " +
        ...
        await reader.ReadAsync(cancellationToken);
        _ = reader.GetInt32(0);
        return (reader.GetInt32(1) != 0, reader.GetInt32(2) != 0, reader.GetInt32(3) != 0,
            reader.GetInt32(4) != 0, reader.GetInt32(5) != 0, reader.GetByte(6) != 0);
    }

    private static async Task<string?> ProbeStagingAsync(
        SqlConnection connection, ConnectionProbeRequest request, ISet<ConnectionCapability> available, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await ExecuteAsync(connection, "CREATE TABLE " + SqlServerIdentifier.Qualified(request.Profile.StagingSchema, name) + " (value int);", cancellationToken);
            created = true;
        ...
        finally
        {
            if (created)
            {
                try
                {
                    await ExecuteAsync(connection, "DROP TABLE " + SqlServerIdentifier.Qualified(request.Profile.StagingSchema, name) + ";", cancellationToken);
                    if (verified)
                        available.Add(request.Role is ConnectionRole.Source
    ...
    {
        const string sql = "SELECT CASE WHEN EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON s.schema_id=t.schema_id WHERE s.name=@schema AND t.name=@table) THEN CAST(1 AS bit) ELSE CAST(0 AS bit) END;";
        await using var command = new SqlCommand(sql, connection) { CommandTimeout = 5 };
