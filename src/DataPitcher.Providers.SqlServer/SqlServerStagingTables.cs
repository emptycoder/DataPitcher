...
public sealed class SqlServerStagingTables : IAsyncDisposable
{
    ...
    public Task ReplaceSourceCandidatesAsync(TableDefinition t, IReadOnlyCollection<StableKey> keys, CancellationToken ct) =>
        ReplaceAsync(_source, InputTableName(t), t, keys, ct);

    public Task ReplaceTargetCandidatesAsync(TableDefinition t, IReadOnlyCollection<StableKey> keys, CancellationToken ct) =>
    ...
    public async Task<int> GenerationAsync(TableDefinition t, StableKey key, CancellationToken ct)
    {
        ...
        await using var command = new SqlCommand($"SELECT [__generation] FROM {Qualified(SourceTableName(t))} WHERE {predicate}", connection);
    ...
    public async Task<int> KeyColumnCountInUniqueIndexAsync(TableDefinition t, CancellationToken ct)
    {
        ...
        await using var command = new SqlCommand(
            "SELECT COUNT(*) FROM sys.index_columns WHERE object_id=OBJECT_ID(@table) AND index_id=(SELECT index_id FROM sys.indexes WHERE object_id=OBJECT_ID(@table) AND name=@index)", connection);
    ...
    private async Task ReplaceAsync(string cs, string name, TableDefinition t, IReadOnlyCollection<StableKey> keys, CancellationToken ct)
    {
    ...
    private async Task EnsureAsync(string cs, string name, TableDefinition t, CancellationToken ct)
    {
        ...
        var declarations = string.Join(", ", columns.Select((column, i) => $"[k{i}] {metadata.Column(column).StoreType} NOT NULL"));
        var index = "UX_" + name;
    ...
    private async Task BulkCopyAsync(string cs, string name, TableDefinition t, IReadOnlyCollection<StableKey> keys, CancellationToken ct)
    {
        var columns = Columns(t);
        var metadata = _schema.Table(t.Name);
        ...
        await connection.OpenAsync(ct);
        using var bulk = new SqlBulkCopy(connection, SqlBulkCopyOptions.Default, null) { DestinationTableName = Qualified(name), EnableStreaming = true };
        foreach (DataColumn column in data.Columns)
            bulk.ColumnMappings.Add(column.ColumnName, column.ColumnName);
        await bulk.WriteToServerAsync(data, ct);
    }

    private async Task<IReadOnlyCollection<StableKey>> InsertNewAsync(TableDefinition t, int generation, CancellationToken ct)
    {
        var columns = Columns(t);
        ...
        var sql = $"INSERT {Qualified(SourceTableName(t))} ({names},[__generation]) " +
                  $"OUTPUT {inserted} " +
    ...
    private static Task DropAsync(string cs, string name) => ExecuteAsync(cs, "DROP TABLE IF EXISTS " + Qualified(name), CancellationToken.None);
}
