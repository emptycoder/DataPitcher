...
public sealed class SqlServerBatchStageWriter
{
    public async Task StageAsync(SqlConnection connection, SqlTransaction transaction, SqlServerExecutionContext context, SqlServerWriteTable table, SqlServerTransferBatch batch, CancellationToken cancellationToken)
    {
        ...
        var declarations = string.Join(",", new[] { "[job_id] uniqueidentifier NOT NULL", "[run_id] uniqueidentifier NOT NULL", "[fence_token] bigint NOT NULL", "[batch_sequence] bigint NOT NULL" }.Concat(columns.Select(column => SqlServerIdentifier.Quote(column.Name) + " " + column.StoreType + (column.IsNullable ? " NULL" : " NOT NULL"))));
        await using (var create = new SqlCommand("IF SCHEMA_ID(N'datapitcher') IS NULL EXEC(N'CREATE SCHEMA [datapitcher]'); IF OBJECT_ID(N'" + stage.Replace("'", "''", StringComparison.Ordinal) + "',N'U') IS NULL CREATE TABLE " + stage + " (" + declarations + ");", connection, transaction))
        ...
        foreach (var column in columns) data.Columns.Add(column.Name, column.ClrType);
        foreach (var row in batch.Rows)
        {
            ...
            foreach (var column in columns) values[column.Name] = row.Values[column.Name] ?? DBNull.Value;
            data.Rows.Add(values);
        }

        using var reader = data.CreateDataReader();
        using var bulk = new SqlBulkCopy(connection, SqlBulkCopyOptions.Default, transaction) { DestinationTableName = stage, BatchSize = 0, BulkCopyTimeout = 30, EnableStreaming = true };
        for (var ordinal = 0; ordinal < data.Columns.Count; ordinal++) bulk.ColumnMappings.Add(ordinal, data.Columns[ordinal].ColumnName);
