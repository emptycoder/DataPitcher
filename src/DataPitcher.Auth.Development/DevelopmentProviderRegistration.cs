...
public sealed class DevelopmentProviderRegistration : IAuthProviderRegistration
{
    ...
    public DevelopmentProviderRegistration(IHostEnvironment environment, DevelopmentProviderOptions options) { if (!environment.IsDevelopment() && !environment.IsEnvironment("Test")) throw new InvalidOperationException("Development authentication is allowed only in Development or Test."); this.options = options; if (string.IsNullOrWhiteSpace(options.SigningKey)) throw new InvalidOperationException("Development signing key is required. Set Authentication__Development__SigningKey to a local value of at least 32 bytes."); if (Encoding.UTF8.GetByteCount(options.SigningKey) < 32) throw new ArgumentException("Development signing key must be at least 32 bytes.", nameof(options)); }
