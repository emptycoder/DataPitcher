...
{
    public PostgreSqlTransferBatch(long sequence, IEnumerable<PostgreSqlTransferRow> rows, StableKey lastStableKey, PostgreSqlConflictPolicy policy)
    {
        Sequence = sequence;
...
public static class PostgreSqlStableKeyCodec
{
    public static byte[] Encode(StableKey key, PostgreSqlWriteTable table)
    {
        ...
        foreach (var column in table.StableKeyColumns)
        {
            ...
            if (value is null) throw new ArgumentException("Stable-key values cannot be null.");
    ...
    public static StableKey Decode(byte[] bytes, PostgreSqlWriteTable table)
    {
        ...
        foreach (var column in table.StableKeyColumns) parts.Add(new(column.Name, Read(bytes, ref offset, column.ProviderType)));
    ...
    private static void Write(ArrayBufferWriter<byte> buffer, object value, NpgsqlDbType type)
    {
        ...
        if (type == NpgsqlDbType.Integer && value is int integer) { BinaryPrimitives.WriteInt32BigEndian(span, integer); buffer.Advance(4); return; }
        if (type == NpgsqlDbType.Bigint && value is long bigInteger) { BinaryPrimitives.WriteInt64BigEndian(span, bigInteger); buffer.Advance(8); return; }
        var text = type == NpgsqlDbType.Text && value is string stringValue ? Encoding.UTF8.GetBytes(stringValue) : throw new NotSupportedException($"Stable-key type {type} is not supported.");
    ...
    private static object Read(byte[] bytes, ref int offset, NpgsqlDbType type)
    {
        ...
        if (type == NpgsqlDbType.Integer) { offset += 4; return BinaryPrimitives.ReadInt32BigEndian(span); }
        if (type == NpgsqlDbType.Bigint) { offset += 8; return BinaryPrimitives.ReadInt64BigEndian(span); }
