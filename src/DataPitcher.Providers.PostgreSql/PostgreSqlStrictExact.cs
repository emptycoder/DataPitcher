...
public sealed class PostgreSqlStrictExact(NpgsqlDataSource dataSource)
{
    public async Task EnsureAvailableAsync(PostgreSqlWriteTable table, CancellationToken cancellationToken)
    {
        ...
        if (await ExistsAsync("SELECT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid=@target::regclass AND NOT tgisinternal AND tgenabled <> 'D')", target, cancellationToken))
            throw new PostgreSqlStrictExactBlockedException("StrictExact is blocked by a target trigger.");
        if (await ExistsAsync("SELECT EXISTS (SELECT 1 FROM pg_rewrite WHERE ev_class=@target::regclass AND rulename <> '_RETURN')", target, cancellationToken))
            throw new PostgreSqlStrictExactBlockedException("StrictExact is blocked by a target rewrite rule.");
        if (await ExistsAsync("SELECT EXISTS (SELECT 1 FROM pg_constraint WHERE confrelid=@target::regclass AND contype='f' AND confupdtype IN ('c','n','d'))", target, cancellationToken))
...
public sealed class PostgreSqlSequenceRealigner(NpgsqlDataSource dataSource)
{
    public async Task RealignAsync(PostgreSqlWriteTable table, string column, CancellationToken cancellationToken)
    {
        ...
        await using (var sequence = new NpgsqlCommand(sequenceSql, connection, transaction))
        {
            ...
            if (await reader.ReadAsync(cancellationToken))
                owned = new(reader.GetString(0), reader.GetFieldValue<uint>(1), reader.GetInt64(2), reader.GetBoolean(3), reader.GetFieldValue<uint>(4), reader.GetInt16(5));
        }
        ...
        const string sharedSql = "SELECT EXISTS (SELECT 1 FROM pg_depend dep LEFT JOIN pg_attrdef def ON def.oid=dep.objid AND dep.classid='pg_attrdef'::regclass WHERE dep.refobjid=@sequence AND (def.oid IS NULL OR def.adrelid<>@table OR def.adnum<>@column))";
        ...
        long last;
        bool called;
        await using (var current = new NpgsqlCommand("SELECT last_value,is_called FROM " + name, connection, transaction))
        {
        ...
        await using var extreme = new NpgsqlCommand("SELECT " + (increment > 0 ? "max" : "min") + "(" + PostgreSqlIdentifier.Quote(column) + ") FROM " + PostgreSqlIdentifier.Qualified(table.Target.Schema, table.Target.Name), connection, transaction);
        ...
        await using var set = new NpgsqlCommand("SELECT setval(@sequence::regclass,@value,true)", connection, transaction);
