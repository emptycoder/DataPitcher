...
public sealed class PostgreSqlSelectionExecutor(NpgsqlDataSource source, PostgreSqlSchemaSnapshot schema)
    : ISelectionExecutor
{
    public async Task ValidateAsync(GeneratedSelectionSql selection, CancellationToken cancellationToken)
    {
        if (selection.IsRawSql)
            RawSqlSafetyValidator.Validate(RawSqlDialect.PostgreSql, selection.CommandText);
        await using var command = Command(
            "/* DataPitcher.Selection.Validate */ SELECT * FROM ("
    ...
    )
    {
        ...
        await using var command = Command(
            "/* DataPitcher.Selection.Keys */ SELECT DISTINCT "
                + aliases
                + " FROM ("
                + CommandText(selection)
                + "\n) AS selection ORDER BY "
                + aliases
                + " LIMIT @take",
            selection
        );
    ...
    )
    {
        ...
        var projection = string.Join(
            ", ",
            keys.Select(PostgreSqlIdentifier.Quote)
        ...
        var join = string.Join(
            ...
            selection.RootStableKey.Columns.Select(
                (column, index) =>
                    ...
                    + " = keys."
                    + PostgreSqlIdentifier.Quote(keys[index])
            )
        );
        var order = string.Join(
            ", ",
            selection.RootStableKey.Columns.Select(column => rootAlias + "." + PostgreSqlIdentifier.Quote(column))
        );
        var sql =
            "/* DataPitcher.Selection.Preview */ WITH selection AS ("
            + CommandText(selection)
            + "\n), keys AS (SELECT DISTINCT "
            + aliases
            + " FROM selection ORDER BY "
            + aliases
            + " LIMIT @previewLimit) SELECT "
            + projection
            + " FROM "
            + PostgreSqlIdentifier.Qualified(root.Schema, root.Name)
            + " AS "
            + rootAlias
            + " INNER JOIN keys ON "
            + join
    ...
    public async Task<long> CountAsync(GeneratedSelectionSql selection, CancellationToken cancellationToken)
    {
        var aliases = Aliases(selection.RootStableKey);
        await using var command = Command(
            "/* DataPitcher.Selection.Count */ SELECT count(*) FROM (SELECT DISTINCT "
                + aliases
                + " FROM ("
                + CommandText(selection)
                + "\n) AS selection) AS keys",
            selection
        );
        ...
        return column.ClrType == typeof(string)
            ...
            : column.ClrType == typeof(byte[])
                ? "CASE WHEN octet_length("
                    ...
                    + value
                    + " FROM 1 FOR @binaryLimit) ELSE "
                    + value
                    + " END, COALESCE(octet_length("
                    + value
                    + ") > @binaryLimit, false)"
            : value + ", false";
    }
