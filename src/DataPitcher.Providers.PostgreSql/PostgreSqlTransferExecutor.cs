...
public sealed class PostgreSqlTransferExecutor
{
    ...
    public PostgreSqlTransferExecutor(NpgsqlDataSource target, IDerivedCheckpointMirror mirror, IAfterTargetCommitBarrier barrier)
    {
        _target = target;
        _mirror = mirror;
        _barrier = barrier;
    ...
    public async Task<PostgreSqlBatchCommit> ExecuteAsync(PostgreSqlExecutionContext context, PostgreSqlWriteTable table, PostgreSqlTransferBatch batch, CancellationToken cancellationToken)
    {
        ...
        await new PostgreSqlBatchStageWriter().StageAsync(connection, transaction, context, table, batch, cancellationToken);
        var result = await new PostgreSqlBatchApplier().ApplyAsync(connection, transaction, context, table, batch, cancellationToken);
    ...
    public async Task<PostgreSqlResumePoint> RecoverAsync(PostgreSqlExecutionContext context, PostgreSqlWriteTable table, CancellationToken cancellationToken)
    {
        ...
        await _mirror.WriteAsync(checkpoint, cancellationToken);
        return new PostgreSqlResumePoint(checkpoint.LastBatchSequence + 1, checkpoint.LastBatchSequence < 0 ? null : PostgreSqlStableKeyCodec.Decode(checkpoint.LastStableKey, table));
    }
}
