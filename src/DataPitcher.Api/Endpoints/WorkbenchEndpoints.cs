...
public static class WorkbenchEndpoints
{
    public static void Map(IEndpointRouteBuilder app)
    {
        ...
        WithStandardProblems(selections.MapGet("/workbench-schema", GetWorkbenchSchemaAsync)).RequireAuthorization(ApiPolicyNames.SelectionsRead);
        WithStandardProblems(selections.MapPost("/compile", CompileAsync)).RequireAuthorization(ApiPolicyNames.SelectionsRead);
        WithStandardProblems(selections.MapPost("/preview", PreviewAsync)).RequireAuthorization(ApiPolicyNames.SelectionsRead);
        WithStandardProblems(selections.MapPost("/count", CountAsync)).RequireAuthorization(ApiPolicyNames.SelectionsRead);
        WithStandardProblems(selections.MapPost("/save", SaveAsync)).RequireAuthorization(ApiPolicyNames.SelectionsWrite);
    ...
    private static async Task<Ok<SelectionWorkbenchSchemaResponse>> GetWorkbenchSchemaAsync(SchemaSnapshotStore snapshots, CancellationToken cancellationToken)
    {
        var snapshot = await snapshots.GetLatestAsync(cancellationToken);
        if (snapshot is null) return TypedResults.Ok(new SelectionWorkbenchSchemaResponse([], [], ""));

        var tables = snapshot.Content.Tables.Select(table => new SelectionTableResponse(
            TableId(table.Schema, table.Name),
            table.Schema,
            table.Name,
            null,
            table.PrimaryKey?.Columns.ToArray(),
            table.Columns.Select(column => new SelectionColumnResponse(column.Name, ValueKindOf(column.ClrType))).ToArray())).ToArray();

        var foreignKeys = snapshot.Content.ForeignKeys
            .Select(foreignKey => new ForeignKeyPathResponse(foreignKey.Name, TableId(foreignKey.ChildTable.Schema, foreignKey.ChildTable.Name), TableId(foreignKey.ParentTable.Schema, foreignKey.ParentTable.Name)))
            .ToArray();
    ...
    private static Task<Ok<CompilationResponse>> CompileAsync(SelectionRequestBody request)
    {
        ...
        var parameters = request.Parameters.Select(parameter => new TypedParameterDefinitionResponse(parameter.Name, parameter.Kind)).ToArray();
        return Task.FromResult(TypedResults.Ok(new CompilationResponse(request.RawSql, parameters, [], request.SchemaRevision)));
    }

    private static Task<PreviewResponse> PreviewAsync(SelectionRequestBody request) => throw new SelectionExecutionNotWiredException();
    ...
    private static async Task<Ok<ListSelectionsResponse>> ListAsync(SelectionStore selections, CancellationToken cancellationToken)
    {
        var records = await selections.ListAsync(cancellationToken);
        var saved = records.Select(record => new SavedSelectionResponse(record.SelectionId, record.DisplayName, record.Version, ETag(record.Version), Mode(record.QueryJson), [])).ToArray();
    ...
    private static string ValueKindOf(string clrType) => clrType switch
    {
