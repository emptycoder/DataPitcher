...
public static class JobEventStream
{
    ...
    private static async Task<IResult> StreamAsync(
        ...
        IJobEventReader reader,
        IJobEventSignal signal,
        IValidatedAccessTokenLifetime lifetime,
        [FromHeader(Name = "Last-Event-ID")] string? lastEventIdHeader,
        CancellationToken cancellationToken
    )
    {
        if (
            request.Query.Keys.Any(IsCredentialQueryKey) || !TryParseLastEventId(lastEventIdHeader, out var lastEventId)
        )
        ...
        if (!authorization.Succeeded)
            return AuthorizationFailureDiagnostics.IsIndeterminate(authorization.Failure)
        ...
        try
        {
            ...
            return new JobEventStreamResult(
                jobId,
                lastEventId ?? 0,
                page,
                reader,
                signal,
                lifetime.GetExpiryUtc(user),
                cancellationToken
            );
        }
    ...
    private static bool TryParseLastEventId(string? header, out long? lastEventId)
    {
        ...
        if (header is null)
            return true;
        if (!long.TryParse(header, out var value) || value < 0)
            return false;
        lastEventId = value;
        return true;
    }

    private sealed class JobEventStreamResult(
        Guid jobId,
        long lastEventId,
        JobEventPage initialPage,
        IJobEventReader reader,
        IJobEventSignal signal,
