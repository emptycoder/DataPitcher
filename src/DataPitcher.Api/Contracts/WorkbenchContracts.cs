...
public sealed record PlanSchemaDependencyGraphRelationshipResponse(
    string Id,
    string Name,
    string ChildTableId,
...
public sealed record SelectionTableResponse(
    string TableId,
    string SchemaName,
    string TableName,
    long? ApproximateRowCount,
    IReadOnlyList<string>? StableKeyColumns,
    IReadOnlyList<SelectionColumnResponse> Columns
);

public sealed record ForeignKeyPathResponse(string ForeignKeyId, string ChildTableId, string ParentTableId);

public sealed record SelectionWorkbenchSchemaResponse(
    IReadOnlyList<SelectionTableResponse> Tables,
    IReadOnlyList<ForeignKeyPathResponse> ForeignKeys,
...
public sealed record SelectionRequestBody(
    ...
    object? Visual,
    string? RawSql,
    IReadOnlyList<TypedParameterValueRequest> Parameters,
    string SchemaRevision,
...
public sealed record CompilationResponse(
    string SqlSnapshot,
    IReadOnlyList<TypedParameterDefinitionResponse> Parameters,
...
public sealed record SavedSelectionResponse(
    Guid SelectionId,
    string DisplayName,
    long Version,
    string ETag,
...
/// </summary>
public sealed class SelectionExecutionNotWiredException()
