...
public static class ApiProblemMapper
{
    ...
    public static ProblemDetails Map(ApiFault fault, string correlationId)
    {
        ...
        var problem = new ProblemDetails { Status = definition.Status, Title = definition.Title, Detail = definition.Detail, Type = definition.Type };
    ...
    private static string CorrelationId(HttpContext context) =>
        Guid.TryParse(context.Request.Headers["X-Correlation-ID"], out _) ? context.Request.Headers["X-Correlation-ID"].ToString() : Guid.NewGuid().ToString();

    private static Definition DefinitionFor(ApiErrorClass errorClass) => errorClass switch
    {
        ...
        _ => new(500, "internal_error", "Internal error", "The operation could not be completed.", "urn:datapitcher:internal"),
    };
}
...
public sealed class ApiExceptionHandler : IExceptionHandler
{
    ...
    private static ApiErrorClass Classify(HttpContext context, Exception exception) => exception switch
    {
