...
public static class AuthorizationFailureDiagnostics
{
    ...
    public static bool IsIndeterminate(AuthorizationFailure? failure) =>
        failure?.FailureReasons.Any(reason => string.Equals(reason.Message, IndeterminateReason, StringComparison.Ordinal)) == true;
}

public sealed class PermissionAuthorizationHandler(IPermissionDecisionResolver resolver) : AuthorizationHandler<PermissionRequirement>
{
    protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, PermissionRequirement requirement)
    {
        var decision = resolver.Resolve(context.User, requirement.Permission);
        if (decision.Outcome == AuthorizationOutcome.Granted) context.Succeed(requirement);
        else if (decision.Outcome == AuthorizationOutcome.Indeterminate) context.Fail(new AuthorizationFailureReason(this, AuthorizationFailureDiagnostics.IndeterminateReason));
        return Task.CompletedTask;
    }
}

public sealed class ResourceAuthorizationHandler(IResourceAccessGrantReader grants, IPermissionDecisionResolver? resolver = null) : AuthorizationHandler<ResourcePermissionRequirement, ApiResource>
{
    private readonly IPermissionDecisionResolver _resolver = resolver ?? new ClaimsPermissionDecisionResolver();

    protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, ResourcePermissionRequirement requirement, ApiResource resource)
    {
        var decision = _resolver.Resolve(context.User, requirement.Permission);
        if (decision.Outcome == AuthorizationOutcome.Indeterminate) { context.Fail(new AuthorizationFailureReason(this, AuthorizationFailureDiagnostics.IndeterminateReason)); return; }
        if (decision.Outcome != AuthorizationOutcome.Granted) return;
        if (await grants.IsGrantedAsync(context.User, resource, CancellationToken.None)) context.Succeed(requirement);
    }
}
...
public sealed class ApiAuthorizationMiddlewareResultHandler : IAuthorizationMiddlewareResultHandler
{
    public async Task HandleAsync(RequestDelegate next, HttpContext context, AuthorizationPolicy policy, PolicyAuthorizationResult authorizeResult)
    {
        ...
        if (AuthorizationFailureDiagnostics.IsIndeterminate(authorizeResult.AuthorizationFailure)) { await ApiAuthorizationResults.Indeterminate(context).ExecuteAsync(context); return; }
