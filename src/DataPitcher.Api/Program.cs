        ...
        (operation, context, _) =>
        {
            ...
            if (metadata.OfType<IAllowAnonymous>().Any())
            {
                if (metadata.OfType<AnonymousAccessJustificationMetadata>().FirstOrDefault() is { } justification)
                {
                    ...
                    operation.Extensions["x-datapitcher-anonymous-justification"] = new JsonNodeExtension(
                        JsonValue.Create(justification.Reason)!
                    );
                }
            }
...
app.MapGet("/health/live", () => Results.Ok(new { status = "live" }))
    ...
    .AllowAnonymousWithJustification(
        "Liveness must be reachable before authentication infrastructure is confirmed healthy."
    );

app.MapGet(
        "/api/providers",
        () =>
            Results.Ok<IReadOnlyList<ProviderResponse>>([
    ...
    .ProducesProblem(StatusCodes.Status500InternalServerError)
    .AllowAnonymousWithJustification(
        "Provider identifiers and display names are non-sensitive and needed before sign-in to build the connection form."
    );
