        ...
        IJobEventReader
{
    public Task<JobEvent> AppendAsync(JobEventAppend append, CancellationToken cancellationToken)
    {
        ...
        db.Execute(
            ...
            new DataParameter("job", append.JobId.ToString())
        );
        var eventId = db.Query<long>(
                "SELECT NextEventId FROM JobEventStreams WHERE JobId = @job",
                new DataParameter("job", append.JobId.ToString())
            )
            .Single();
        ...
        db.Execute(
            ...
            new DataParameter("next", eventId + 1),
            new DataParameter("job", append.JobId.ToString())
        );
        db.Execute(
            ...
            new DataParameter("job", append.JobId.ToString()),
            new DataParameter("event", eventId),
            new DataParameter("type", append.EventType),
            new DataParameter("state", append.Payload.State),
            new DataParameter("rows", append.Payload.RowsTransferred),
            new DataParameter("bytes", append.Payload.BytesTransferred),
            new DataParameter("occurred", occurredAtUtc.ToString("O", CultureInfo.InvariantCulture))
        );
        transaction.Commit();
    ...
    public async Task<JobEventPage> ReadAfterAsync(Guid jobId, long? lastEventId, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        using var db = database.Open();
        var stream = await db.GetTable<JobEventStreamRow>()
            .SingleOrDefaultAsync(row => row.JobId == jobId.ToString(), cancellationToken);
        ...
        var events = await db.GetTable<JobEventRow>()
            .Where(row => row.JobId == jobId.ToString() && row.EventId > cursor)
            .OrderBy(row => row.EventId)
            .ToArrayAsync(cancellationToken);
        return new(
            events
                .Select(row => new JobEvent(
                    Guid.Parse(row.JobId),
                    row.EventId,
                    row.EventType,
                    new(row.State, row.RowsTransferred, row.BytesTransferred),
    ...
    public Task TrimBeforeAsync(Guid jobId, long oldestAvailableEventId, CancellationToken cancellationToken)
    {
        ...
        db.Execute(
            "INSERT OR IGNORE INTO JobEventStreams (JobId, NextEventId, OldestAvailableEventId) VALUES (@job, 1, 1)",
            new DataParameter("job", jobId.ToString())
        );
        var nextEventId = db.Query<long>(
                "SELECT NextEventId FROM JobEventStreams WHERE JobId = @job",
                new DataParameter("job", jobId.ToString())
            )
            .Single();
        ...
        db.Execute(
            ...
            new DataParameter("job", jobId.ToString()),
            new DataParameter("oldest", oldestAvailableEventId)
        );
        db.Execute(
    ...
    private sealed class JobEventStreamRow
    {
        ...
        [Column]
        public long OldestAvailableEventId { get; set; }
    }
