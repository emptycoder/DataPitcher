...
public enum GroupResolutionState
{
    NotApplicable,
    Complete,
    Indeterminate,
...
public sealed class GroupResolutionResult
{
    ...
    {
        State = state;
        ImmutableGroupIds = Array.AsReadOnly(
            immutableGroupIds.Distinct(StringComparer.Ordinal).OrderBy(id => id, StringComparer.Ordinal).ToArray()
        );
    }
    ...
    public static GroupResolutionResult Complete(IEnumerable<string> immutableGroupIds) =>
        new(GroupResolutionState.Complete, immutableGroupIds);
