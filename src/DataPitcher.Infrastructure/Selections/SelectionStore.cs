...
public sealed record SelectionRecord(Guid SelectionId, string DisplayName, string QueryJson, long Version, DateTimeOffset UpdatedUtc);
...
{
    public Task<SelectionRecord> SaveAsync(Guid selectionId, string displayName, string queryJson, string ifMatch, CancellationToken cancellationToken)
    {
        ...
        if (existing is null)
        {
            var row = new SelectionRow { SelectionId = selectionId.ToString(), DisplayName = displayName, QueryJson = queryJson, Version = 1, CreatedUtc = now, UpdatedUtc = now };
            db.Insert(row);
            transaction.Commit();
        ...
        var affected = db.Execute(
            "UPDATE Selections SET DisplayName = @displayName, QueryJson = @queryJson, Version = Version + 1, UpdatedUtc = @updatedUtc WHERE SelectionId = @selectionId AND Version = @version",
            new DataParameter("displayName", displayName), new DataParameter("queryJson", queryJson), new DataParameter("updatedUtc", now),
        ...
        transaction.Commit();
        return Task.FromResult(new SelectionRecord(selectionId, displayName, queryJson, existing.Version + 1, clock.UtcNow));
    }
    ...
    private static SelectionRecord ToRecord(SelectionRow row) =>
        new(Guid.Parse(row.SelectionId), row.DisplayName, row.QueryJson, row.Version, DateTimeOffset.Parse(row.UpdatedUtc, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind));
