...
public sealed class EntraPrincipalNormalizer(EntraProviderOptions options) : IExternalPrincipalNormalizer
{
    public AuthenticatedProviderPrincipal Normalize(ClaimsPrincipal principal, string validatedIssuer)
    {
        ...
        var kind = principal.FindFirst("idtyp")?.Value switch
        {
            null => PrincipalKind.User,
            "user" => PrincipalKind.User,
            "app" => PrincipalKind.ServicePrincipal,
            _ => throw new InvalidOperationException("Validated Entra token has an unsupported idtyp claim."),
        };
        var key = new ExternalPrincipalKey(options.ProviderInstance, validatedIssuer, tenant, kind, objectId);
        var presentation = new PrincipalPresentation(
        ...
        return new AuthenticatedProviderPrincipal(
            new NormalizedPrincipal(key, presentation),
            principal
                .FindAll("roles")
    ...
    private static string RequiredGuid(ClaimsPrincipal principal, string type) =>
        Guid.TryParse(principal.FindFirst(type)?.Value, out var value)
            ? value.ToString()
            : throw new InvalidOperationException($"Validated Entra token has no GUID {type} claim.");
