...
public sealed class GenericOpenIdConnectOptions
{
    ...
    public PrincipalKind PrincipalKind { get; init; }
    public string? GroupClaimType { get; init; }
...
public sealed class GenericOpenIdConnectProviderRegistration : IAuthProviderRegistration
{
    ...
    public GenericOpenIdConnectProviderRegistration(IConfigurationSection configuration)
    {
        options =
            configuration.Get<GenericOpenIdConnectOptions>()
            ?? throw new ArgumentException("Generic OIDC configuration is required.", nameof(configuration));
        Validate(options);
    }
    ...
    public void Register(AuthenticationBuilder builder)
    {
        builder.Services.AddKeyedSingleton<IExternalPrincipalNormalizer>(
            SchemeName,
            new GenericOpenIdConnectPrincipalNormalizer(options)
        );
        builder.AddJwtBearer(
            SchemeName,
            bearer =>
            {
                bearer.Authority = options.Issuer;
                ...
                bearer.Events = new JwtBearerEvents
                {
                    OnTokenValidated = context =>
                    {
                        ...
                        if (options.RequiredScopes.Except(scopes, StringComparer.Ordinal).Any())
    ...
    private static void Validate(GenericOpenIdConnectOptions options)
    {
        if (
            ...
            throw new ArgumentException(
                "Generic OIDC scheme, provider instance, absolute issuer, and audience are required.",
                nameof(options)
            );
        if (options.RequiredScopes.Any(string.IsNullOrWhiteSpace))
            throw new ArgumentException("Required scopes cannot be empty.", nameof(options));
    }
}
