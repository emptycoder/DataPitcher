...
public enum CycleStrategy
{
    ...
    SuspendAndRevalidateConstraints,
    Blocked,
}
...
public sealed record ConnectionFingerprint(
    string Provider,
...
public sealed class RelationshipPolicy
{
    ...
    public TableAddress From { get; }
    public TableAddress To { get; }
...
{
    [JsonConstructor]
    public StableKeyDefinition(TableAddress table, string constraintName, IReadOnlyList<string> columns)
    {
        Table = table;
...
{
    [JsonConstructor]
    public TableMapping(TableAddress source, TableAddress target, IReadOnlyList<ColumnMapping> columns)
    {
        Source = source;
        Target = target;
        Columns = Array.AsReadOnly(columns.ToArray());
    ...
    public TableAddress Source { get; }
...
{
    [JsonConstructor]
    public TopologicalGroup(IReadOnlyList<TableAddress> tables) => Tables = Array.AsReadOnly(tables.ToArray());
...
public sealed class PlanTable
{
    ...
    public PlanTableState State { get; }
...
public sealed class TransferPlanContent
{
    ...
    public TransferPlanContent(
        ...
        SchemaSnapshotReference sourceSchema,
        SchemaSnapshotReference targetSchema,
        IReadOnlyList<SelectionReference> selections,
        IReadOnlyList<RelationshipPolicy> relationships,
        IReadOnlyList<TableConflictPolicy> conflictPolicies,
        ConsistencyMode consistencyMode,
        TransferMode transferMode,
        TriggerStrategy triggerStrategy,
        ConstraintStrategy constraintStrategy,
        IReadOnlyList<StableKeyDefinition> stableKeys,
    ...
    )
    {
        ...
        ConflictPolicies = Array.AsReadOnly(conflictPolicies.ToArray());
        ConsistencyMode = consistencyMode;
        TransferMode = transferMode;
        TriggerStrategy = triggerStrategy;
        ConstraintStrategy = constraintStrategy;
        StableKeys = Array.AsReadOnly(stableKeys.ToArray());
    ...
    public ConnectionFingerprint Target { get; }
    public SchemaSnapshotReference SourceSchema { get; }
    public SchemaSnapshotReference TargetSchema { get; }
    public IReadOnlyList<SelectionReference> Selections { get; }
    public IReadOnlyList<RelationshipPolicy> Relationships { get; }
    public IReadOnlyList<TableConflictPolicy> ConflictPolicies { get; }
    public ConsistencyMode ConsistencyMode { get; }
    public TransferMode TransferMode { get; }
    public TriggerStrategy TriggerStrategy { get; }
    public ConstraintStrategy ConstraintStrategy { get; }
    public IReadOnlyList<StableKeyDefinition> StableKeys { get; }
    public IReadOnlyList<PlanTable> Tables { get; }
