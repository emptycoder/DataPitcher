...
{
    [JsonConstructor] public RelationshipPolicy(string name, TableAddress from, TableAddress to, IReadOnlyList<string> fromColumns, IReadOnlyList<string> toColumns, RelationshipDirection direction, bool isEnabled)
...
{
    [JsonConstructor] public StableKeyDefinition(TableAddress table, string constraintName, IReadOnlyList<string> columns) { Table = table; ConstraintName = constraintName; Columns = Array.AsReadOnly(columns.ToArray()); }
...
{
    [JsonConstructor] public TableMapping(TableAddress source, TableAddress target, IReadOnlyList<ColumnMapping> columns) { Source = source; Target = target; Columns = Array.AsReadOnly(columns.ToArray()); }
...
{
    [JsonConstructor] public TopologicalGroup(IReadOnlyList<TableAddress> tables) => Tables = Array.AsReadOnly(tables.ToArray());
...
{
    [JsonConstructor] public TransferPlanContent(ConnectionFingerprint source, ConnectionFingerprint target, SchemaSnapshotReference sourceSchema, SchemaSnapshotReference targetSchema, IReadOnlyList<SelectionReference> selections, IReadOnlyList<RelationshipPolicy> relationships, IReadOnlyList<TableConflictPolicy> conflictPolicies, ConsistencyMode consistencyMode, TransferMode transferMode, TriggerStrategy triggerStrategy, ConstraintStrategy constraintStrategy, IReadOnlyList<StableKeyDefinition> stableKeys, IReadOnlyList<PlanTable> tables, BatchTarget batchTarget, VerificationStrategy verificationStrategy, ManifestCounts manifestTotals)
