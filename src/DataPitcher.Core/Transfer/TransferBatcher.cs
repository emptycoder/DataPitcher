...
public sealed class TransferBatcher
{
    ...
    public TransferBatcher(TransferPipelineOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
        _options = options;
    }

    public async IAsyncEnumerable<TransferBatch> ReadBatchesAsync(
    ...
    )
    {
        ...
        await foreach (var sourceRow in source.ReadAsync(cancellationToken).WithCancellation(cancellationToken))
        {
            ...
            var row = converter is null ? sourceRow : await converter.ConvertAsync(sourceRow, cancellationToken);
            if (rows.Count > 0 && row.PayloadBytes > _options.BatchTarget.MaximumBytes - bytes)
            {
                yield return new TransferBatch(sequence++, rows);
                rows = [];
                bytes = 0;
            }
            rows.Add(row);
