...
{
    public RawSqlSelection(TableDefinition rootTable, UniqueConstraint rootStableKey, string commandText, IEnumerable<SelectionSqlParameter> parameters)
    {
        RootTable = rootTable; RootStableKey = rootStableKey; CommandText = commandText; Parameters = Array.AsReadOnly(parameters.ToArray());
    }
...
public sealed record SelectionExecutionLimits(int MaximumResultSize, TimeSpan ValidationTimeout, TimeSpan KeyTimeout, TimeSpan PreviewTimeout, TimeSpan CountTimeout)
{
    ...
    public static SelectionExecutionLimits Default { get; } = new(100000, TimeSpan.FromSeconds(5), TimeSpan.FromSeconds(30), TimeSpan.FromSeconds(15), TimeSpan.FromSeconds(30));
    public void Validate()
    {
        if (MaximumResultSize < 1) throw new ArgumentOutOfRangeException(nameof(MaximumResultSize));
...
public sealed record SelectionExecutionRequest(string SchemaSnapshotHash, SelectionQuery? Query, RawSqlSelection? RawSql, SelectionExecutionLimits Limits)
{
    ...
    {
        Limits.Validate();
        if ((Query is null) == (RawSql is null)) throw new ArgumentException("Selection execution requires exactly one query mode.");
    }
}
...
public sealed class SelectionKeySet
{
    ...
    {
        RootTable = rootTable; Keys = Array.AsReadOnly(keys.ToArray());
    }
    public TableDefinition RootTable { get; }
...
public sealed class SelectionPreviewRow
{
    ...
    {
        StableKey = stableKey;
        Values = new ReadOnlyDictionary<string, SelectionPreviewCell>(new Dictionary<string, SelectionPreviewCell>(values, StringComparer.Ordinal));
    }
...
public sealed class SelectionPreview
{
    ...
    {
        Columns = Array.AsReadOnly(columns.ToArray()); Rows = Array.AsReadOnly(rows.ToArray());
    }
...
{
    public static string For(int ordinal) => ordinal < 0 ? throw new ArgumentOutOfRangeException(nameof(ordinal)) : "__datapitcher_key_" + ordinal;
    public static IReadOnlyList<string> ForKey(UniqueConstraint key) => Array.AsReadOnly(Enumerable.Range(0, key.Columns.Count).Select(For).ToArray());
}
...
{
    public static SelectionCountCacheKey Create(string schemaHash, GeneratedSelectionSql sql) => new(schemaHash, Hash(sql.CommandText), Hash(string.Join("\u001f", sql.Parameters.Select(parameter => parameter.Name + "\u001e" + parameter.ClrType.FullName + "\u001e" + System.Text.Json.JsonSerializer.Serialize(parameter.Value, parameter.ClrType)))), string.Join("\u001f", sql.RootStableKey.Columns));
...
public interface ISelectionExecutor
{
    ...
    Task<SelectionKeySet> ReadKeysAsync(GeneratedSelectionSql selection, int maximumResultSize, CancellationToken cancellationToken);
    Task<SelectionPreview> PreviewAsync(GeneratedSelectionSql selection, int rowLimit, int textLimit, int binaryLimit, CancellationToken cancellationToken);
