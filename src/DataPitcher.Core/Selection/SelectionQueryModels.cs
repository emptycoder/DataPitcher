...
public sealed record SelectionParameterValue
{
    public SelectionParameterValue(Type clrType, object value)
    {
        if (value is null || value.GetType() != clrType)
            throw new ArgumentException("Selection parameter value must have its declared non-null CLR type.");
        ClrType = clrType;
        Value = value;
    }
...
public sealed record ForeignKeyJoin(
    string FromAlias,
    string Alias,
    ForeignKeyDefinition ForeignKey,
    RelationshipDirection Direction
) : SelectionJoin(FromAlias, Alias);

public sealed record ManualJoin(
    string FromAlias,
    string Alias,
    TableDefinition Table,
    IReadOnlyList<SelectionColumnPair> Pairs
) : SelectionJoin(FromAlias, Alias);
...
public sealed record ComparisonPredicate(
    ...
    SelectionComparison Operator,
    SelectionParameterValue Value
) : SelectionPredicate;

public sealed record BetweenPredicate(
    SelectionColumn Column,
    SelectionParameterValue Lower,
    SelectionParameterValue Upper
) : SelectionPredicate;

public sealed record SetPredicate(SelectionColumn Column, bool Negated, IReadOnlyList<SelectionParameterValue> Values)
    : SelectionPredicate;

public sealed record NullPredicate(SelectionColumn Column, bool Negated) : SelectionPredicate;

public sealed record TextPredicate(SelectionColumn Column, TextMatch Match, SelectionParameterValue Value)
    : SelectionPredicate;

public sealed record BooleanPredicate(SelectionColumn Column, SelectionParameterValue Value) : SelectionPredicate;

public sealed record TemporalRangePredicate(
    SelectionColumn Column,
    TemporalKind Kind,
    SelectionParameterValue Lower,
    SelectionParameterValue Upper
...
public sealed record ExistsPredicate(
    TableDefinition Table,
    string Alias,
    IReadOnlyList<SelectionCorrelation> Correlations,
    SelectionPredicate? Predicate,
    bool Negated
) : SelectionPredicate;

public sealed class SelectionSchema
{
    public SelectionSchema(IEnumerable<TableDefinition> tables, IEnumerable<ForeignKeyDefinition> foreignKeys)
    {
...
public sealed class SelectionQuery
{
    public SelectionQuery(
        ...
        SelectionTableReference root,
        StableKeySelection rootStableKey,
        IEnumerable<SelectionJoin> joins,
        SelectionPredicate? predicate
    )
    {
        Schema = schema;
        Root = root;
        RootStableKey = rootStableKey;
        Joins = Array.AsReadOnly(joins.ToArray());
        Predicate = predicate;
        SelectionQueryValidator.Validate(this);
    }

    public SelectionSchema Schema { get; }
    public SelectionTableReference Root { get; }
    public StableKeySelection RootStableKey { get; }
    public IReadOnlyList<SelectionJoin> Joins { get; }
