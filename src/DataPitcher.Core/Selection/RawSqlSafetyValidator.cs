...
public static class RawSqlSafetyValidator
{
    ...
    public static void Validate(RawSqlDialect dialect, string sql)
    {
        ...
        if (dialect == RawSqlDialect.SqlServer && tokens.Any(token => EqualsToken(token.Text, "GO"))) throw new RawSqlValidationException("SQL Server batch separators are not allowed.");
        if (tokens.Count == 0 || (!EqualsToken(tokens[0].Text, "SELECT") && !EqualsToken(tokens[0].Text, "WITH"))) throw new RawSqlValidationException("Raw SQL must start with SELECT or WITH.");

        var separators = tokens.Select((token, index) => (token, index)).Where(pair => string.Equals(pair.token.Text, ";", StringComparison.Ordinal)).ToArray();
        ...
        {
            if (Forbidden.Contains(token.Text)) throw new RawSqlValidationException("Raw SQL contains a data-modifying token: " + token.Text.ToUpperInvariant() + ".");
            if (EqualsToken(token.Text, "INTO")) throw new RawSqlValidationException("Raw SQL contains a data-modifying token: INTO.");
        }
    }
    ...
    {
        var tokens = Tokens(sql);
        var terminator = tokens.LastOrDefault(token => token.Depth == 0 && string.Equals(token.Text, ";", StringComparison.Ordinal));
        var end = terminator?.Start ?? sql.Length;
        var order = tokens.TakeWhile(token => token.Start < end).Select((token, index) => (token, index)).LastOrDefault(pair =>
            pair.token.Depth == 0 &&
        ...
        return order.token is not null ? sql[..order.token.Start].TrimEnd() : terminator is not null ? sql[..terminator.Start].TrimEnd() : sql;
    }
    ...
    private static List<Token> Tokens(string sql)
    {
        ...
        for (int index = 0, depth = 0; index < sql.Length;)
        {
            ...
            if (value == '(') { depth++; index++; continue; }
            if (value == ')') { depth--; index++; continue; }
            if (value == ';') { result.Add(new Token(";", index, depth)); index++; continue; }
