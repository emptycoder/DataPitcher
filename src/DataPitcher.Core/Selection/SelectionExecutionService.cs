...
public sealed class SelectionExecutionService(
    ISelectionSqlCompiler compiler,
    ISelectionExecutor executor,
    SelectionCountCache counts
)
{
    public async Task<SelectionKeySet> ExecuteKeysAsync(
        SelectionExecutionRequest request,
    ...
    )
    {
        ...
        return await BoundedAsync(
            "keys",
            request.Limits.KeyTimeout,
            token => executor.ReadKeysAsync(sql, request.Limits.MaximumResultSize, token),
            cancellationToken
        );
    }
    ...
    )
    {
        ...
        return await BoundedAsync(
            "preview",
            request.Limits.PreviewTimeout,
            token =>
                executor.PreviewAsync(
                    sql,
                    SelectionExecutionLimits.PreviewRowLimit,
                    SelectionExecutionLimits.PreviewTextLength,
                    SelectionExecutionLimits.PreviewBinaryLength,
                    token
                ),
            cancellationToken
        );
    ...
    )
    {
        ...
        if (counts.TryGet(key, out var cached))
            return cached;
        var count = await BoundedAsync(
            "count",
            request.Limits.CountTimeout,
            token => executor.CountAsync(sql, token),
            cancellationToken
        );
    ...
    private async Task<GeneratedSelectionSql> PrepareAsync(
        SelectionExecutionRequest request,
        PermissionSet permissions,
        CancellationToken cancellationToken
    )
    {
        ...
        await BoundedAsync(
            "validation",
            request.Limits.ValidationTimeout,
            async token =>
            {
                await executor.ValidateAsync(sql, token);
                return true;
            },
            cancellationToken
        );
    ...
    )
    {
        ...
        {
            throw new SelectionOperationTimeoutException(operation);
        }
    }
}
